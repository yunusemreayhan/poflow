...
) -> ApiResult<Vec<db::Notification>> {
    let limit = q
        .get("limit")
        .and_then(|l| l.parse().ok())
        .unwrap_or(50i64)
        .min(200);
    db::list_notifications(&engine.pool, claims.user_id, limit)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn unread_count(
    ...
    let count = db::unread_count(&engine.pool, claims.user_id)
        .await
        .map_err(internal)?;
...
pub async fn mark_notifications_read(
    ...
    db::mark_read(&engine.pool, claims.user_id, id)
        .await
        .map_err(internal)?;
