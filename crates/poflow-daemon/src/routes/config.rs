...
pub async fn update_config(
    ...
    if cfg.work_duration_min == 0 || cfg.work_duration_min > 240 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "work_duration_min must be 1-240",
        ));
    }
    if cfg.short_break_min == 0 || cfg.short_break_min > 60 {
        return Err(err(StatusCode::BAD_REQUEST, "short_break_min must be 1-60"));
    }
    if cfg.long_break_min == 0 || cfg.long_break_min > 120 {
        return Err(err(StatusCode::BAD_REQUEST, "long_break_min must be 1-120"));
    }
    if cfg.long_break_interval == 0 || cfg.long_break_interval > 20 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "long_break_interval must be 1-20",
        ));
    }
    if cfg.daily_goal > 50 {
        return Err(err(StatusCode::BAD_REQUEST, "daily_goal must be 0-50"));
    }
    if !["hours", "points"].contains(&cfg.estimation_mode.as_str()) {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "estimation_mode must be 'hours' or 'points'",
        ));
    }
    if !["dark", "light"].contains(&cfg.theme.as_str()) {
        return Err(err(
            StatusCode::BAD_REQUEST,
    ...
    if cfg.auto_archive_days > 3650 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "auto_archive_days must be 0-3650",
        ));
    }
    ...
    db::set_user_config(&engine.pool, claims.user_id, &uc)
        .await
        .map_err(internal)?;
