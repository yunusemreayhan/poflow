...
) -> Result<(StatusCode, Json<db::Team>), ApiError> {
    if req.name.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Team name cannot be empty"));
    }
    if req.name.len() > 100 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Team name too long (max 100 chars)",
        ));
    }
    // V4: Limit total teams
    let teams = db::list_teams(&engine.pool).await.map_err(internal)?;
    if teams.len() >= 50 {
        return Err(err(StatusCode::BAD_REQUEST, "Too many teams (max 50)"));
    }
    let team = db::create_team(&engine.pool, req.name.trim())
        .await
        .map_err(internal)?;
    db::add_team_member(&engine.pool, team.id, claims.user_id, "admin")
        .await
        .map_err(internal)?;
...
pub async fn get_team(
    ...
    _claims: Claims,
    Path(id): Path<i64>,
) -> ApiResult<db::TeamDetail> {
    db::get_team_detail(&engine.pool, id)
        .await
        .map(Json)
        .map_err(|_| err(StatusCode::NOT_FOUND, "Team not found"))
}
...
pub async fn delete_team(
    ...
    if !auth::is_admin_or_root(&claims)
        && !db::is_team_admin(&engine.pool, id, claims.user_id)
            .await
            .map_err(internal)?
    {
        return Err(err(
            StatusCode::FORBIDDEN,
            "Only root or team admin can delete teams",
        ));
    }
...
pub async fn add_team_member(
    ...
    if !db::is_team_admin(&engine.pool, id, claims.user_id)
        .await
        .map_err(internal)?
    ...
    db::get_user(&engine.pool, req.user_id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "User not found"))?;
    db::add_team_member(&engine.pool, id, req.user_id, &req.role)
        .await
        .map_err(internal)?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn remove_team_member(
    ...
    db::remove_team_member(&engine.pool, id, user_id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "User is not a team member"))?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn get_my_teams(
    ...
    db::get_user_teams(&engine.pool, claims.user_id)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn add_team_root_tasks(
    State(engine): State<AppState>,
    claims: Claims,
    Path(id): Path<i64>,
    ...
    for tid in &task_ids {
        db::add_team_root_task(&engine.pool, id, *tid)
            .await
            .map_err(internal)?;
    }
    Ok(StatusCode::NO_CONTENT)
...
pub async fn remove_team_root_task(
    State(engine): State<AppState>,
    claims: Claims,
    ...
    db::remove_team_root_task(&engine.pool, id, task_id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Task not a root task in this team"))?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn get_team_scope(
    ...
    let detail = db::get_team_detail(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Team not found"))?;
    if detail.root_task_ids.is_empty() {
        return Ok(Json(vec![]));
    }
    db::get_descendant_ids(&engine.pool, &detail.root_task_ids)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn snapshot_sprint(
    State(engine): State<AppState>,
    claims: Claims,
    Path(id): Path<i64>,
) -> ApiResult<db::SprintDailyStat> {
    ...
    db::snapshot_sprint(&engine.pool, id)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn get_sprint_board(
    ...
    _claims: Claims,
    Path(id): Path<i64>,
) -> ApiResult<db::SprintBoard> {
    db::get_sprint(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Sprint not found"))?;
    db::get_sprint_board(&engine.pool, id)
        .await
        .map(Json)
        .map_err(internal)
}
