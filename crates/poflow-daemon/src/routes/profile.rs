...
pub async fn update_profile(
    ...
    if let Some(ref p) = req.password {
        let current_pw = req.current_password.as_ref().ok_or_else(|| {
            err(
                StatusCode::BAD_REQUEST,
                "current_password is required to change password",
            )
        })?;
        let user = db::get_user(&engine.pool, claims.user_id)
            .await
            .map_err(internal)?;
        let hash_check = user.password_hash.clone();
        let pw_check = current_pw.clone();
        let valid = tokio::task::spawn_blocking(move || {
            bcrypt::verify(&pw_check, &hash_check).unwrap_or(false)
        })
        .await
        .map_err(internal)?;
        if !valid {
    ...
    if let Some(ref u) = req.username {
        validate_username(u)?;
        db::update_username(&engine.pool, claims.user_id, u)
            .await
            .map_err(|e| {
    ...
    if let Some(ref p) = req.password {
        ...
        let hash = tokio::task::spawn_blocking(move || bcrypt::hash(&pw, 12))
            ...
            .map_err(internal)?
            .map_err(internal)?;
        db::update_user_password(&engine.pool, claims.user_id, &hash)
            .await
            .map_err(internal)?;
    ...
    if let Some(ref email) = req.email {
        ...
        let val = if email_trimmed.is_empty() {
            None
        } else {
            Some(email_trimmed)
        };
    ...
    if let Some(ref tz) = req.timezone {
        ...
        let val = if tz_trimmed.is_empty() {
            None
        } else {
            Some(tz_trimmed)
        };
    ...
    let refresh_token =
        auth::create_refresh_token(user.id, &user.username, &user.role).map_err(internal)?;
...
pub async fn get_notif_prefs(
    State(engine): State<AppState>,
    claims: Claims,
) -> ApiResult<Vec<NotifPref>> {
    let rows: Vec<(String, bool)> =
        sqlx::query_as("SELECT event_type, enabled FROM notification_prefs WHERE user_id = ?")
            .bind(claims.user_id)
            .fetch_all(&engine.pool)
            .await
            .map_err(internal)?;
    let map: std::collections::HashMap<String, bool> = rows.into_iter().collect();
    let prefs = EVENT_TYPES
        .iter()
        .map(|e| NotifPref {
            event_type: e.to_string(),
            enabled: *map.get(*e).unwrap_or(&true),
        })
        .collect();
    Ok(Json(prefs))
}
...
pub async fn update_notif_prefs(
    State(engine): State<AppState>,
    claims: Claims,
    Json(prefs): Json<Vec<NotifPref>>,
) -> Result<StatusCode, ApiError> {
    if prefs.len() > EVENT_TYPES.len() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            format!("Too many prefs (max {})", EVENT_TYPES.len()),
        ));
    }
    for p in &prefs {
        if !EVENT_TYPES.contains(&p.event_type.as_str()) {
            return Err(err(
                StatusCode::BAD_REQUEST,
                format!("Unknown event type: {}", p.event_type),
            ));
        }
