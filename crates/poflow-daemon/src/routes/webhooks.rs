...
pub async fn list_webhooks(
    ...
    claims: Claims,
) -> ApiResult<Vec<db::Webhook>> {
    db::list_webhooks(&engine.pool, claims.user_id)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn create_webhook(
...
) -> Result<(StatusCode, Json<db::Webhook>), ApiError> {
        ...
        if let Some(host) = url.host_str() {
            ...
            let is_blocked = blocked.contains(&host)
                ...
                || host
                    ...
                    .map(|ip| crate::webhook::is_private_ip_pub(&ip))
                    .unwrap_or(false);
            if is_blocked {
                return Err(err(
                    StatusCode::BAD_REQUEST,
                    "Webhook URL must not point to private/loopback addresses",
                ));
            }
        }
    }
    ...
    let (count,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM webhooks WHERE user_id = ?")
        ...
        .map_err(internal)?;
    if count >= 50 {
        return Err(err(StatusCode::BAD_REQUEST, "Too many webhooks (max 50)"));
    }
    if events != "*" {
...
pub async fn delete_webhook(
    ...
    db::delete_webhook(&engine.pool, id, claims.user_id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Webhook not found"))?;
...
pub async fn update_webhook(
    ...
    if req.events.is_some() {
        sql_parts.push("events = ?");
    }
    if req.active.is_some() {
        sql_parts.push("active = ?");
    }
    ...
    if let Some(active) = req.active {
        q = q.bind(if active { 1i64 } else { 0 });
    }
    q = q.bind(id).bind(claims.user_id);
    q.execute(&engine.pool).await.map_err(internal)?;
    let updated = sqlx::query_as::<_, db::Webhook>("SELECT * FROM webhooks WHERE id = ?")
        .bind(id)
        .fetch_one(&engine.pool)
        .await
        .map_err(internal)?;
...
pub async fn list_webhook_deliveries(
...
) -> ApiResult<Vec<db::WebhookDelivery>> {
    let wh: (i64,) = sqlx::query_as("SELECT user_id FROM webhooks WHERE id = ?")
        .bind(id)
        .fetch_one(&engine.pool)
        .await
    ...
    db::list_deliveries(&engine.pool, id, limit)
        .await
        .map(Json)
        .map_err(internal)
}
