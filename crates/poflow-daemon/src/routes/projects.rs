...
pub async fn list_projects(
    ...
    _claims: Claims,
) -> ApiResult<Vec<db::Project>> {
    db::list_projects(&engine.pool)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn get_project(
    ...
    db::get_project(&engine.pool, id)
        .await
        .map(Json)
        .map_err(|_| err(StatusCode::NOT_FOUND, "Project not found"))
}
...
pub async fn create_project(
...
) -> Result<(StatusCode, Json<db::Project>), ApiError> {
    ...
    let key = req
        ...
        .unwrap_or_else(|| generate_key(&name));
    if key.is_empty() || key.len() > 10 {
        return Err(err(StatusCode::BAD_REQUEST, "Key must be 1-10 characters"));
    }
    if !key
        .chars()
        .all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_')
    {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Key must be alphanumeric (with - or _)",
        ));
    }
    ...
    .map_err(|e| {
        if e.to_string().contains("UNIQUE") {
            err(StatusCode::CONFLICT, "Project key already exists")
        } else {
            internal(e)
        }
    })?;
...
pub async fn delete_project(
    ...
    db::delete_project(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Project not found"))?;
