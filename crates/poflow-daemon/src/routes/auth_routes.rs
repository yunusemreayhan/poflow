...
pub async fn register(
    ...
    let allowed = match std::env::var("POFLOW_ALLOW_REGISTRATION") {
        Ok(v) => v != "0" && v != "false",
    ...
    let hash = tokio::task::spawn_blocking(move || bcrypt::hash(&pw, 12))
        ...
        .map_err(internal)?
        .map_err(internal)?;
    let user = db::create_user(&engine.pool, &req.username, &hash, "user")
        .await
...
pub async fn login(
    ...
    check_auth_rate_limit(&headers)?;
    let user = db::get_user_by_username(&engine.pool, &req.username)
        .await
    ...
    if current_cost < 12 {
        ...
        if let Ok(new_hash) = tokio::task::spawn_blocking(move || bcrypt::hash(&pw2, 12))
            .await
            .map_err(internal)?
        {
            // S2: Use rehash (no password_changed_at update) to avoid invalidating the fresh token
            db::rehash_user_password(&engine.pool, user.id, &new_hash)
                .await
                .ok();
        }
    }
...
pub async fn change_password(
    ...
    let new_hash = tokio::task::spawn_blocking(move || bcrypt::hash(&new_pw, 12))
        ...
        .map_err(internal)?
        .map_err(internal)?;
    db::update_user_password(&engine.pool, claims.user_id, &new_hash)
        .await
        .map_err(internal)?;
...
pub async fn refresh_token(
    ...
    let claims = auth::verify_token(&req.refresh_token)
    ...
    let user = db::get_user(&engine.pool, claims.user_id)
        .await
        .map_err(|_| err(StatusCode::UNAUTHORIZED, "User not found"))?;
    let token = auth::create_token(user.id, &user.username, &user.role).map_err(internal)?;
    let refresh_token =
        auth::create_refresh_token(user.id, &user.username, &user.role).map_err(internal)?;
    Ok(Json(AuthResponse {
