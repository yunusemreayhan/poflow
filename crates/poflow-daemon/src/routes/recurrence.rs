...
pub async fn set_recurrence(
    ...
    claims: Claims,
    Path(id): Path<i64>,
    Json(req): Json<SetRecurrenceRequest>,
) -> ApiResult<db::TaskRecurrence> {
    ...
    if !VALID_PATTERNS.contains(&req.pattern.as_str()) {
        return Err(err(
            ...
            "Pattern must be: daily, weekly, biweekly, monthly",
        ));
    }
    ...
    db::set_recurrence(&engine.pool, id, &req.pattern, &req.next_due)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn get_recurrence(
    ...
    _claims: Claims,
    Path(id): Path<i64>,
) -> ApiResult<Option<db::TaskRecurrence>> {
    db::get_task(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Task not found"))?;
    db::get_recurrence(&engine.pool, id)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn remove_recurrence(
    ...
    db::remove_recurrence(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "No recurrence set for this task"))?;
