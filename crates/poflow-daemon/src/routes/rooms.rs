...
pub async fn list_rooms(
    ...
    claims: Claims,
) -> ApiResult<Vec<db::Room>> {
    // B4: Non-root users only see rooms they're a member of — use db function
    if claims.role == "root" {
        db::list_rooms(&engine.pool)
            .await
            .map(Json)
            .map_err(internal)
    } else {
        db::list_user_rooms(&engine.pool, claims.user_id)
            .await
            .map(Json)
            .map_err(internal)
    }
...
pub async fn create_room(
...
) -> Result<(StatusCode, Json<db::Room>), ApiError> {
    if req.name.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Room name cannot be empty"));
    }
    if req.name.len() > 200 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Room name too long (max 200 chars)",
        ));
    }
    let unit = req.estimation_unit.as_deref().unwrap_or("points");
    if !["points", "hours", "mandays", "tshirt"].contains(&unit) {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "estimation_unit must be points, hours, mandays, or tshirt",
        ));
    }
    let room_type = req.room_type.as_deref().unwrap_or("estimation");
    if room_type != "estimation" {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "room_type must be 'estimation'",
        ));
    }
    if req.project.as_ref().map(|p| p.len()).unwrap_or(0) > 200 {
        return Err(err(
            StatusCode::BAD_REQUEST,
    ...
    let (count,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM rooms WHERE creator_id = ? AND status != 'closed'")
            .bind(claims.user_id)
    ...
    if count >= 20 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Too many active rooms (max 20)",
        ));
    }
    let r = db::create_room(
        &engine.pool,
        &req.name,
        room_type,
        unit,
...
pub async fn get_room_state(
    ...
    claims: Claims,
    Path(id): Path<i64>,
) -> ApiResult<db::RoomState> {
    // S2: Verify membership (or root)
    let state = db::get_room_state(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Room not found"))?;
    if !auth::is_admin_or_root(&claims)
        && !state.members.iter().any(|m| m.username == claims.username)
    {
...
pub async fn delete_room(
    ...
    let room = db::get_room(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Room not found"))?;
    if !is_owner_or_root(room.creator_id, &claims) {
...
pub async fn join_room(
    ...
    db::get_room(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Room not found"))?;
    db::join_room(&engine.pool, id, claims.user_id)
        .await
        .map_err(internal)?;
    engine.notify(ChangeEvent::Rooms);
    Ok(StatusCode::NO_CONTENT)
...
pub async fn leave_room(
    ...
    let room = db::get_room(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Room not found"))?;
    if room.creator_id == claims.user_id {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Room creator cannot leave — delete the room instead",
        ));
    }
    db::leave_room(&engine.pool, id, claims.user_id)
        .await
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Not a member of this room"))?;
    engine.notify(ChangeEvent::Rooms);
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn kick_member(
    ...
    if !db::is_room_admin(&engine.pool, id, claims.user_id)
        .await
        .map_err(internal)?
        && !auth::is_admin_or_root(&claims)
    {
        return Err(err(StatusCode::FORBIDDEN, "Admin only"));
    }
    let uid = db::get_user_id_by_username(&engine.pool, &username)
        .await
        .map_err(internal)?
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "User not found"))?;
    if uid == claims.user_id {
        return Err(err(StatusCode::BAD_REQUEST, "Cannot kick yourself"));
    }
    db::leave_room(&engine.pool, id, uid)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "User is not a member of this room"))?;
...
pub async fn set_room_role(
    ...
    if !VALID_ROOM_ROLES.contains(&req.role.as_str()) {
        return Err(err(
            StatusCode::BAD_REQUEST,
            format!(
                "Invalid room role '{}'. Must be one of: {}",
                req.role,
                VALID_ROOM_ROLES.join(", ")
            ),
        ));
    }
    let uid = db::get_user_id_by_username(&engine.pool, &req.username)
        ...
        .map_err(internal)?
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "User not found"))?;
    db::set_room_member_role(&engine.pool, id, uid, &req.role)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "User is not a member of this room"))?;
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn start_voting(
    ...
    let task = db::get_task(&engine.pool, req.task_id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Task not found"))?;
    if task.deleted_at.is_some() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Cannot vote on a deleted task",
        ));
    }
    db::start_voting(&engine.pool, id, req.task_id)
        .await
        .map(|r| {
            engine.notify(ChangeEvent::Rooms);
            Json(r)
...
pub async fn cast_vote(
    ...
    if req.value < 0.0 || req.value > 1000.0 {
        return Err(err(StatusCode::BAD_REQUEST, "Vote value must be 0-1000"));
    }
    let room = db::get_room(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Room not found"))?;
    if room.status != "voting" {
        return Err(err(StatusCode::BAD_REQUEST, "Room is not in voting state"));
    }
    // Verify user is a room member
    let members = db::get_room_members(&engine.pool, id)
        .await
        .map_err(internal)?;
    ...
    if members
        .iter()
        .any(|m| m.user_id == claims.user_id && m.role == "observer")
    {
        return Err(err(StatusCode::FORBIDDEN, "Observers cannot vote"));
    }
    let task_id = room
        .current_task_id
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "No active vote"))?;
    db::cast_vote(&engine.pool, id, task_id, claims.user_id, req.value)
        .await
        .map_err(internal)?;
    engine.notify(ChangeEvent::Rooms);
    Ok(StatusCode::NO_CONTENT)
...
pub async fn accept_estimate(
    State(engine): State<AppState>,
    claims: Claims,
    Path(id): Path<i64>,
    Json(req): Json<AcceptEstimateRequest>,
) -> ApiResult<db::Task> {
    if !db::is_room_admin(&engine.pool, id, claims.user_id)
        .await
        .map_err(internal)?
    ...
    if req.value < 0.0 || req.value > 10000.0 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Estimate value must be 0-10000",
        ));
    }
    let room = db::get_room(&engine.pool, id)
        .await
    ...
    if room.status != "revealed" {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Votes must be revealed before accepting",
        ));
    }
    let task_id = room
        .current_task_id
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "No active vote"))?;
    let task = db::accept_estimate(&engine.pool, id, task_id, req.value, &room.estimation_unit)
        .await
        .map_err(internal)?;
    ...
    let voted_task_ids: std::collections::HashSet<i64> =
        state.vote_history.iter().map(|v| v.task_id).collect();
    ...
    let has_children: std::collections::HashSet<i64> = all_tasks
        .iter()
        .filter_map(|t| t.parent_id)
        .filter(|pid| room_task_ids.contains(pid))
        .collect();
    let next = all_tasks
        .iter()
    ...
    if let Some(next_task) = next {
        db::start_voting(&engine.pool, id, next_task.id)
            .await
            .map_err(internal)?;
    } else {
        db::set_room_status(&engine.pool, id, "lobby")
            .await
            .map_err(internal)?;
    }
    engine.notify(ChangeEvent::Rooms);
...
pub async fn close_room(
    ...
    db::set_room_status(&engine.pool, id, "closed")
        .await
        .map_err(internal)?;
    engine.notify(ChangeEvent::Rooms);
    Ok(StatusCode::NO_CONTENT)
...
pub async fn export_room_history(
    ...
    axum::response::Response::builder()
        ...
        .header(
            "content-disposition",
            format!("attachment; filename=\"room_{}_history.json\"", id),
        )
...
pub async fn room_ws(
    ...
    let members = db::get_room_members(&engine.pool, room_id)
        .await
        .map_err(internal)?;
