...
) -> ApiResult<Vec<db::StatusTransition>> {
    db::list_status_transitions(&engine.pool, q.project_id)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn create_transition(
...
) -> Result<(StatusCode, Json<db::StatusTransition>), super::ApiError> {
    ...
    if req.from_status.trim().is_empty() || req.to_status.trim().is_empty() {
        return Err(err(
            ...
            "from_status and to_status are required",
        ));
    }
    ...
    let t = db::create_status_transition(
        &engine.pool,
        req.from_status.trim(),
        req.to_status.trim(),
        req.project_id,
    )
    .await
    .map_err(|e| {
        if e.to_string().contains("UNIQUE") {
            err(StatusCode::CONFLICT, "Transition already exists")
        } else {
            internal(e)
        }
    })?;
...
pub async fn delete_transition(
    ...
    db::delete_status_transition(&engine.pool, id)
        .await
        .map_err(internal)?;
