...
pub async fn get_dependencies(
    ...
    db::get_dependencies(&engine.pool, id)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn add_dependency(
    ...
    db::get_task(&engine.pool, req.depends_on)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Dependency task not found"))?;
    db::add_dependency(&engine.pool, id, req.depends_on)
        .await
...
pub async fn remove_dependency(
    ...
    db::remove_dependency(&engine.pool, id, dep_id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Dependency not found"))?;
    engine.notify(ChangeEvent::Tasks);
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn get_all_dependencies(
    State(engine): State<AppState>,
    _claims: Claims,
) -> ApiResult<Vec<db::TaskDependency>> {
    db::get_all_dependencies(&engine.pool)
        .await
        .map(Json)
        .map_err(internal)
}
