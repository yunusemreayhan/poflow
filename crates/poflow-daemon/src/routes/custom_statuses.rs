...
pub async fn list_custom_statuses(
    State(engine): State<AppState>,
    _claims: Claims,
) -> ApiResult<Vec<db::CustomStatus>> {
    db::list_custom_statuses(&engine.pool)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn create_custom_status(
    State(engine): State<AppState>,
    claims: Claims,
    Json(req): Json<CreateCustomStatusRequest>,
) -> Result<(StatusCode, Json<db::CustomStatus>), ApiError> {
    ...
    if name.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Status name cannot be empty"));
    }
    if name.len() > 50 {
        return Err(err(
    ...
    if !name.chars().all(|c| c.is_alphanumeric() || c == '_') {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Status name must be alphanumeric/underscore",
        ));
    }
    ...
    let status = db::create_custom_status(
        &engine.pool,
        &name,
        color,
        category,
    ...
    .map_err(|e| {
        if e.to_string().contains("UNIQUE") {
            err(StatusCode::CONFLICT, "Status name already exists")
        } else {
            internal(e)
        }
    })?;
...
pub async fn update_custom_status(
    ...
    let color = req.color.as_deref().unwrap_or("#6366f1");
    let sort_order = req.sort_order.unwrap_or(0);
    db::update_custom_status(&engine.pool, id, &name, color, category, sort_order)
        .await
        .map(Json)
        .map_err(|e| {
            if e.to_string().contains("not found") {
                err(StatusCode::NOT_FOUND, "Status not found")
            } else {
                internal(e)
            }
        })
...
pub async fn delete_custom_status(
    State(engine): State<AppState>,
    claims: Claims,
    Path(id): Path<i64>,
    ...
    db::delete_custom_status(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Status not found"))?;
