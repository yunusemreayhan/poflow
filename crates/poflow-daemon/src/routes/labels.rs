...
pub async fn create_label(
...
) -> Result<(StatusCode, Json<db::Label>), ApiError> {
    ...
    if req.name.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Label name cannot be empty"));
    }
    if req.name.len() > 100 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Label name too long (max 100)",
        ));
    }
        ...
        || !color[1..].chars().all(|c| c.is_ascii_hexdigit())
    {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Color must be #RGB or #RRGGBB hex format",
        ));
    }
    let label = db::create_label(&engine.pool, req.name.trim(), color)
        .await
        .map_err(|e| {
            if e.to_string().contains("UNIQUE") {
                err(StatusCode::CONFLICT, "Label already exists")
            } else {
                internal(e)
            }
        })?;
...
pub async fn delete_label(
    ...
    db::delete_label(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Label not found"))?;
...
pub async fn update_label(
    ...
    db::update_label(&engine.pool, id, req.name.trim(), color)
        .await
        .map(Json)
        .map_err(|e| {
...
pub async fn add_task_label(
    State(engine): State<AppState>,
    claims: Claims,
    Path((task_id, label_id)): Path<(i64, i64)>,
) -> Result<StatusCode, ApiError> {
    ...
    sqlx::query("SELECT 1 FROM labels WHERE id = ?")
        .bind(label_id)
        .fetch_one(&engine.pool)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Label not found"))?;
    db::add_task_label(&engine.pool, task_id, label_id)
        .await
        .map_err(internal)?;
...
pub async fn remove_task_label(
    ...
    db::remove_task_label(&engine.pool, task_id, label_id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Label not on task"))?;
    engine.notify(ChangeEvent::Tasks);
    Ok(StatusCode::NO_CONTENT)
}
...
pub async fn get_task_labels(
    ...
    db::get_task(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Task not found"))?;
    db::get_task_labels(&engine.pool, id)
        .await
        .map(Json)
        .map_err(internal)
}
