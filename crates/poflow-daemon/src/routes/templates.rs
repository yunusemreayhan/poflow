...
pub async fn list_templates(
    ...
    claims: Claims,
) -> ApiResult<Vec<db::TaskTemplate>> {
    db::list_templates(&engine.pool, claims.user_id)
        .await
        .map(Json)
        .map_err(internal)
}
...
pub async fn create_template(
...
) -> Result<(StatusCode, Json<db::TaskTemplate>), ApiError> {
    ...
    if data.len() > 65536 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Template data too large (max 64KB)",
        ));
    }
    ...
    if count.0 >= 100 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Template limit reached (max 100)",
        ));
    }
    let t = db::create_template(&engine.pool, claims.user_id, req.name.trim(), &data)
        .await
        .map_err(internal)?;
...
pub async fn delete_template(
    ...
    let tmpl: (i64,) = sqlx::query_as("SELECT user_id FROM task_templates WHERE id = ?")
        ...
        .fetch_one(&engine.pool)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Template not found"))?;
    if !is_owner_or_root(tmpl.0, &claims) {
        return Err(err(StatusCode::FORBIDDEN, "Not owner"));
    }
    db::delete_template(&engine.pool, id)
        .await
        .map_err(internal)?;
...
pub async fn update_template(
    ...
    sqlx::query("UPDATE task_templates SET name = ?, data = ? WHERE id = ?")
        ...
        .map_err(internal)?;
    let updated =
        sqlx::query_as::<_, db::TaskTemplate>("SELECT * FROM task_templates WHERE id = ?")
            .bind(id)
            .fetch_one(&engine.pool)
            .await
            .map_err(internal)?;
...
pub async fn instantiate_template(
    ...
    let tmpl = db::get_template(&engine.pool, id)
        .await
        .map_err(|_| err(StatusCode::NOT_FOUND, "Template not found"))?;
    if !is_owner_or_root(tmpl.user_id, &claims) {
        return Err(err(StatusCode::FORBIDDEN, "Not owner"));
    }
    ...
    if let Some(items) = data["checklist"].as_array() {
        for (i, item) in items.iter().enumerate() {
            if let Some(title) = item.as_str().or_else(|| item["title"].as_str()) {
                db::add_checklist_item(&engine.pool, t.id, title, i as i64)
                    .await
                    .ok();
            }
        }
        ...
        for label in labels {
            if let Some(name) = label.as_str().or_else(|| label["name"].as_str()) {
                let lid: Option<(i64,)> = sqlx::query_as("SELECT id FROM labels WHERE name = ?")
                    ...
                    .unwrap_or(None);
                if let Some((lid,)) = lid {
                    db::add_task_label(&engine.pool, t.id, lid).await.ok();
                }
            }
        }
    }
    ...
    if let Some(fields) = data["custom_fields"].as_object() {
        for (field_name, value) in fields {
            let fid: Option<(i64,)> = sqlx::query_as("SELECT id FROM custom_fields WHERE name = ?")
                ...
                .unwrap_or(None);
            if let Some((fid,)) = fid {
                db::set_task_field_value(&engine.pool, t.id, fid, value.as_str())
                    .await
                    .ok();
            }
        }
...
pub async fn save_task_as_template(
    ...
    let checklist = db::list_checklist(&engine.pool, id)
        .await
        .unwrap_or_default();
    ...
    let cf_values = db::get_task_field_values(&engine.pool, id)
        .await
        .unwrap_or_default();
    let mut custom_fields = serde_json::Map::new();
    for fv in &cf_values {
        if let Some(v) = &fv.value {
            custom_fields.insert(fv.field_name.clone(), serde_json::Value::String(v.clone()));
        }
    }
