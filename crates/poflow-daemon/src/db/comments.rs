...
pub async fn add_comment(
    ...
    Ok(
        sqlx::query_as::<_, Comment>(&format!("{} WHERE c.id = ?", COMMENT_SELECT))
            .bind(id)
            .fetch_one(pool)
            .await?,
    )
...
pub async fn list_comments(pool: &Pool, task_id: i64) -> Result<Vec<Comment>> {
    ...
    Ok(sqlx::query_as::<_, Comment>(&format!(
        "{} WHERE c.task_id = ? ORDER BY c.created_at ASC LIMIT 500",
        COMMENT_SELECT
    ))
    .bind(task_id)
    .fetch_all(pool)
...
pub async fn get_task_detail(pool: &Pool, id: i64) -> Result<TaskDetail> {
    ...
    let comments_sql = format!(
        "{} WHERE c.task_id IN ({}) ORDER BY c.task_id, c.created_at ASC",
        COMMENT_SELECT, ph
    );
    ...
    let sessions_sql = format!(
        "{} WHERE s.task_id IN ({}) ORDER BY s.task_id, s.started_at DESC LIMIT 2000",
        SESSION_SELECT, ph
    );
    ...
    // Group by task_id
    let mut comments_map: std::collections::HashMap<i64, Vec<Comment>> =
        std::collections::HashMap::new();
    for c in all_comments {
        comments_map.entry(c.task_id).or_default().push(c);
    }
    let mut sessions_map: std::collections::HashMap<i64, Vec<Session>> =
        std::collections::HashMap::new();
    for s in all_sessions {
        if let Some(tid) = s.task_id {
            sessions_map.entry(tid).or_default().push(s);
        }
    }

    // Build tree from flat list
    // Batch fetch custom field values for all tasks
    let cf_rows = get_task_field_values_batch(pool, &task_ids)
        .await
        .unwrap_or_default();
    let mut cf_map: std::collections::HashMap<i64, Vec<TaskFieldValue>> =
        std::collections::HashMap::new();
    for (tid, fv) in cf_rows {
        cf_map.entry(tid).or_default().push(fv);
    }

    let mut detail_map: std::collections::HashMap<i64, TaskDetail> =
        std::collections::HashMap::new();
    for t in &all_tasks {
        detail_map.insert(
            t.id,
    ...
    let mut child_order: Vec<(i64, Option<i64>)> =
        all_tasks.iter().map(|t| (t.id, t.parent_id)).collect();
    child_order.reverse();
