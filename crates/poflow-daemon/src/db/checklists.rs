...
pub async fn list_checklist(pool: &Pool, task_id: i64) -> Result<Vec<ChecklistItem>> {
    Ok(sqlx::query_as::<_, ChecklistItem>(
...
pub async fn add_checklist_item(
    ...
    let now = now_str();
    let id = sqlx::query(
        "INSERT INTO checklist_items (task_id, title, sort_order, created_at) VALUES (?, ?, ?, ?)",
    )
    .bind(task_id)
    .bind(title)
    .bind(sort_order)
...
pub async fn update_checklist_item(
    ...
    if let Some(t) = title {
        sqlx::query("UPDATE checklist_items SET title = ? WHERE id = ?")
            .bind(t)
            .bind(id)
            .execute(pool)
            .await?;
    ...
    if let Some(c) = checked {
        sqlx::query("UPDATE checklist_items SET checked = ? WHERE id = ?")
            .bind(c)
            .bind(id)
            .execute(pool)
            .await?;
...
pub async fn delete_checklist_item(pool: &Pool, id: i64) -> Result<()> {
    let r = sqlx::query("DELETE FROM checklist_items WHERE id = ?")
        .bind(id)
        .execute(pool)
        .await?;
