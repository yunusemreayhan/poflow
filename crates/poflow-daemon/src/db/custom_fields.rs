...
pub async fn list_custom_fields(pool: &Pool) -> Result<Vec<CustomField>> {
    Ok(
        sqlx::query_as::<_, CustomField>("SELECT * FROM custom_fields ORDER BY sort_order, id")
            .fetch_all(pool)
            .await?,
    )
}
...
pub async fn get_task_field_values(pool: &Pool, task_id: i64) -> Result<Vec<TaskFieldValue>> {
    ...
    Ok(rows
        .into_iter()
        .map(|(fid, fname, ftype, val)| TaskFieldValue {
            field_id: fid,
            field_name: fname,
            field_type: ftype,
            value: val,
...
pub async fn get_task_field_values_batch(
    pool: &Pool,
    task_ids: &[i64],
) -> Result<Vec<(i64, TaskFieldValue)>> {
    ...
    let sql = format!(
        "SELECT cv.task_id, cv.field_id, cf.name as field_name, cf.field_type, cv.value \
         FROM task_custom_values cv JOIN custom_fields cf ON cv.field_id = cf.id \
         WHERE cv.task_id IN ({}) ORDER BY cv.task_id, cf.sort_order, cf.id",
        ph
    );
