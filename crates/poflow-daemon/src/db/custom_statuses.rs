...
pub async fn list_custom_statuses(pool: &Pool) -> Result<Vec<CustomStatus>> {
    Ok(
        sqlx::query_as::<_, CustomStatus>("SELECT * FROM custom_statuses ORDER BY sort_order, id")
            .fetch_all(pool)
            .await?,
    )
}
...
pub async fn update_custom_status(
    ...
    sqlx::query(
        "UPDATE custom_statuses SET name = ?, color = ?, category = ?, sort_order = ? WHERE id = ?",
    )
    .bind(name)
    .bind(color)
    .bind(category)
    ...
    .await?;
    Ok(
        sqlx::query_as::<_, CustomStatus>("SELECT * FROM custom_statuses WHERE id = ?")
            .bind(id)
            .fetch_one(pool)
            .await?,
    )
...
pub async fn delete_custom_status(pool: &Pool, id: i64) -> Result<()> {
    let result = sqlx::query("DELETE FROM custom_statuses WHERE id = ?")
        .bind(id)
        .execute(pool)
        .await?;
...
pub async fn get_custom_status_by_name(pool: &Pool, name: &str) -> Result<Option<CustomStatus>> {
    Ok(
        sqlx::query_as::<_, CustomStatus>("SELECT * FROM custom_statuses WHERE name = ?")
            .bind(name)
            .fetch_optional(pool)
            .await?,
    )
