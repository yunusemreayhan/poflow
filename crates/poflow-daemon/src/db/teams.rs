...
pub async fn create_team(pool: &Pool, name: &str) -> Result<Team> {
    let now = now_str();
    let r = sqlx::query("INSERT INTO teams (name, created_at) VALUES (?,?)")
        .bind(name)
        .bind(&now)
        .execute(pool)
        .await?;
...
pub async fn get_team_detail(pool: &Pool, id: i64) -> Result<TeamDetail> {
    ...
    Ok(TeamDetail {
        team,
        members,
        root_task_ids: root_ids.into_iter().map(|r| r.0).collect(),
    })
}
...
pub async fn add_team_member(pool: &Pool, team_id: i64, user_id: i64, role: &str) -> Result<()> {
    sqlx::query("INSERT OR REPLACE INTO team_members (team_id, user_id, role) VALUES (?,?,?)")
        .bind(team_id)
        .bind(user_id)
        .bind(role)
        .execute(pool)
...
pub async fn remove_team_member(pool: &Pool, team_id: i64, user_id: i64) -> Result<()> {
    let r = sqlx::query("DELETE FROM team_members WHERE team_id = ? AND user_id = ?")
        .bind(team_id)
        .bind(user_id)
        .execute(pool)
        .await?;
...
pub async fn add_team_root_task(pool: &Pool, team_id: i64, task_id: i64) -> Result<()> {
    sqlx::query("INSERT OR IGNORE INTO team_root_tasks (team_id, task_id) VALUES (?,?)")
        .bind(team_id)
        .bind(task_id)
        .execute(pool)
        .await?;
...
pub async fn remove_team_root_task(pool: &Pool, team_id: i64, task_id: i64) -> Result<()> {
    let r = sqlx::query("DELETE FROM team_root_tasks WHERE team_id = ? AND task_id = ?")
        .bind(team_id)
        .bind(task_id)
        .execute(pool)
        .await?;
