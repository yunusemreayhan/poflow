...
pub async fn get_task_sessions(pool: &Pool, task_id: i64) -> Result<Vec<Session>> {
    Ok(sqlx::query_as::<_, Session>(&format!(
        "{} WHERE s.task_id = ? ORDER BY s.started_at DESC LIMIT 200",
        SESSION_SELECT
    ))
    .bind(task_id)
    .fetch_all(pool)
...
pub async fn end_session(pool: &Pool, id: i64, status: &str) -> Result<Session> {
    ...
    let row: (String, String) =
        sqlx::query_as("SELECT started_at, status FROM sessions WHERE id = ?")
            .bind(id)
            .fetch_one(pool)
            .await?;
    if row.1 != "running" {
...
pub async fn recover_interrupted(pool: &Pool) -> Result<Vec<Session>> {
    let sessions: Vec<Session> =
        sqlx::query_as(&format!("{} WHERE s.status = 'running'", SESSION_SELECT))
            .fetch_all(pool)
            .await?;
    if !sessions.is_empty() {
        ...
        for s in &sessions {
            ...
            let duration = (Utc::now().naive_utc() - started).num_seconds().max(0);
            sqlx::query(
                "UPDATE sessions SET status='interrupted', ended_at=?, duration_s=? WHERE id=?",
            )
            .bind(&now)
            .bind(duration)
            .bind(s.id)
...
pub async fn get_history(
    ...
    let task_map: std::collections::HashMap<i64, (Option<i64>, String)> = rows
        .into_iter()
    ...
    Ok(sessions
        ...
        .map(|s| {
            ...
            while let Some(id) = current {
                if let Some((pid, title)) = task_map.get(&id) {
...
pub async fn get_day_stats(pool: &Pool, days: i64, user_id: Option<i64>) -> Result<Vec<DayStat>> {
    let from = (Utc::now().naive_utc() - chrono::Duration::days(days))
