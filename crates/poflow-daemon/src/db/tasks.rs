...
pub fn set_fts5_available(v: bool) {
    FTS5_AVAILABLE.store(v, std::sync::atomic::Ordering::Relaxed);
}
fn fts5_ok() -> bool {
    FTS5_AVAILABLE.load(std::sync::atomic::Ordering::Relaxed)
}
...
fn search_clause() -> &'static str {
    if fts5_ok() {
        ...
        " AND (t.title LIKE ? ESCAPE '\\' OR t.tags LIKE ? ESCAPE '\\' OR t.description LIKE ? ESCAPE '\\')"
    }
}
...
pub fn escape_like_pub(s: &str) -> String {
    escape_like(s)
}
...
async fn populate_team_scope_table(pool: &Pool, ids: &[i64]) -> Result<()> {
    sqlx::query("CREATE TEMP TABLE IF NOT EXISTS _team_scope (id INTEGER PRIMARY KEY)")
        .execute(pool)
        .await?;
...
pub async fn create_task(pool: &Pool, opts: CreateTaskOpts<'_>) -> Result<Task> {
    let now = now_str();
    let max_order: (i64,) = match opts.parent_id {
...
pub async fn get_task(pool: &Pool, id: i64) -> Result<Task> {
    Ok(
        sqlx::query_as::<_, Task>(&format!("{} WHERE t.id = ?", TASK_SELECT))
            .bind(id)
            .fetch_one(pool)
            .await?,
    )
...
pub async fn list_tasks_paged(
    ...
    let team_scope: Option<Vec<i64>> = if let Some(tid) = f.team_id {
        let roots: Vec<(i64,)> =
            sqlx::query_as("SELECT task_id FROM team_root_tasks WHERE team_id = ?")
                .bind(tid)
                .fetch_all(pool)
                .await?;
        if roots.is_empty() {
    ...
    if f.status.is_some() {
        q.push_str(" AND t.status = ?");
    }
    if f.project.is_some() {
        q.push_str(" AND t.project = ?");
    }
    if f.search.is_some() {
        q.push_str(search_clause());
    }
    if f.priority.is_some() {
        q.push_str(" AND t.priority = ?");
    }
    if f.due_before.is_some() {
        q.push_str(" AND t.due_date IS NOT NULL AND t.due_date <= ?");
    }
    if f.due_after.is_some() {
        q.push_str(" AND t.due_date IS NOT NULL AND t.due_date >= ?");
    }
    ...
    if f.label.is_some() {
        q.push_str(" AND EXISTS (SELECT 1 FROM task_labels _tl JOIN labels _l ON _l.id = _tl.label_id WHERE _tl.task_id = t.id AND _l.name = ?)");
    }
    let used_temp = if let Some(ref ids) = team_scope {
        if ids.len() > 500 {
            populate_team_scope_table(pool, ids).await?;
        }
    ...
    if let Some(s) = f.search {
        if fts5_ok() {
            let fts = format!("\"{}\"", s.replace('"', "\"\""));
            query = query.bind(fts);
        } else {
            let like = escape_like(s);
    ...
    if let Some(d) = f.due_before {
        query = query.bind(d);
    }
    if let Some(d) = f.due_after {
        query = query.bind(d);
    }
...
pub async fn update_task(pool: &Pool, id: i64, opts: UpdateTaskOpts<'_>) -> Result<Task> {
    let now = now_str();
    let existing = get_task(pool, id).await?;
    let new_parent = match opts.parent_id {
        Some(p) => p,
        None => existing.parent_id,
    };
    let new_desc = match opts.description {
    ...
    let new_tags = match opts.tags {
        Some(v) => v.map(|s| s.to_string()),
        None => existing.tags,
    };
    let new_due = match opts.due_date {
        Some(v) => v.map(|s| s.to_string()),
        None => existing.due_date,
    };
    let new_wdm = match opts.work_duration_minutes {
        Some(v) => v,
        None => existing.work_duration_minutes,
    };
    let new_eo = match opts.estimate_optimistic {
        Some(v) => v,
        None => existing.estimate_optimistic,
    };
    let new_ep = match opts.estimate_pessimistic {
        Some(v) => v,
        None => existing.estimate_pessimistic,
    };
    sqlx::query("UPDATE tasks SET parent_id=?, title=?, description=?, project=?, project_id=?, tags=?, priority=?, estimated=?, estimated_hours=?, remaining_points=?, due_date=?, status=?, sort_order=?, work_duration_minutes=?, estimate_optimistic=?, estimate_pessimistic=?, updated_at=?, updated_by=? WHERE id=?")
        ...
        .bind(opts.sort_order.unwrap_or(existing.sort_order)).bind(new_wdm).bind(new_eo).bind(new_ep).bind(&now).bind(opts.updated_by).bind(id)
        .execute(pool).await?;
    get_task(pool, id).await
}
...
pub async fn delete_task(pool: &Pool, id: i64) -> Result<()> {
    ...
    let sql = format!(
        "UPDATE tasks SET deleted_at = ?, updated_at = ? WHERE id IN ({}) AND deleted_at IS NULL",
        ph
    );
...
pub async fn increment_task_actual(pool: &Pool, id: i64) -> Result<()> {
    sqlx::query("UPDATE tasks SET actual = actual + 1 WHERE id = ?")
        .bind(id)
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn search_tasks_fts(
    ...
    Ok(rows
        .into_iter()
        .map(|(id, title, desc)| (id, sanitize_snippet(&title), sanitize_snippet(&desc)))
        .collect())
}
...
pub async fn restore_task(pool: &Pool, id: i64) -> Result<()> {
    ...
    let now = now_str();
    let sql = format!(
        "UPDATE tasks SET deleted_at = NULL, updated_at = ? WHERE id IN ({})",
        ph
    );
...
mod tests {
    ...
    fn test_sanitize_snippet_strips_html() {
        assert_eq!(
            sanitize_snippet("<img onerror=alert(1)>"),
            "&lt;img onerror=alert(1)&gt;"
        );
        assert_eq!(sanitize_snippet("<mark>hi</mark>"), "<mark>hi</mark>");
        assert_eq!(
            sanitize_snippet("<script>alert(1)</script> <mark>ok</mark>"),
