...
async fn migrate(pool: &Pool) -> Result<()> {
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS sprint_root_tasks (
        sprint_id   INTEGER NOT NULL REFERENCES sprints(id) ON DELETE CASCADE,
        task_id     INTEGER NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
        PRIMARY KEY (sprint_id, task_id)
    )",
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS team_root_tasks (
        team_id     INTEGER NOT NULL REFERENCES teams(id) ON DELETE CASCADE,
        task_id     INTEGER NOT NULL REFERENCES tasks(id) ON DELETE CASCADE,
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS epic_group_tasks (
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_sessions_user_id ON sessions(user_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_burn_log_task_id ON burn_log(task_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_burn_log_sprint_id ON burn_log(sprint_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_burn_log_user_id ON burn_log(user_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_sprint_tasks_sprint_id ON sprint_tasks(sprint_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_sprint_tasks_task_id ON sprint_tasks(task_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_tasks_user_id ON tasks(user_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_comments_task_id ON comments(task_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_rooms_creator_id ON rooms(creator_id)")
        .execute(pool)
        .await?;
    ...
    .await
    .ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_task_watchers_user ON task_watchers(user_id)")
        .execute(pool)
        .await
        .ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_burn_log_sprint ON burn_log(sprint_id, cancelled)")
        .execute(pool)
        .await
        .ok();
    // Performance indexes for common query patterns
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_tasks_deleted_at ON tasks(deleted_at)")
        .execute(pool)
        .await
        .ok();
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_sessions_task_id ON sessions(task_id)")
        .execute(pool)
        .await
        .ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_task_assignees_task ON task_assignees(task_id)")
        .execute(pool)
        .await
        .ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_task_labels_task ON task_labels(task_id)")
        .execute(pool)
        .await
        .ok();
    sqlx::query(
    ...
    .await
    .ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_checklist_items_task ON checklist_items(task_id)")
        .execute(pool)
        .await
        .ok();
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_audit_log_created_at ON audit_log(created_at)")
        ...
        .await
        .ok();
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_task_links_task ON task_links(task_id)")
        .execute(pool)
        .await
        .ok();
    // F7: Composite indexes for common query patterns
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_tasks_deleted_user ON tasks(deleted_at, user_id)")
        .execute(pool)
        .await
        .ok();
    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_tasks_deleted_project ON tasks(deleted_at, project)",
    )
    .execute(pool)
    .await
    .ok();
    ...
    if !applied_set.contains(&1) {
        if let Err(e) = sqlx::query("ALTER TABLE sprints ADD COLUMN retro_notes TEXT")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE sprints ADD COLUMN retro_notes TEXT", e);
        }
    ...
    if !applied_set.contains(&2) {
        if let Err(e) = sqlx::query("ALTER TABLE tasks ADD COLUMN deleted_at TEXT")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE tasks ADD COLUMN deleted_at TEXT", e);
        }
    ...
    if !applied_set.contains(&4) {
        if let Err(e) = sqlx::query("ALTER TABLE sprints ADD COLUMN capacity_hours REAL")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE sprints ADD COLUMN capacity_hours REAL", e);
        }
    ...
    if !applied_set.contains(&5) {
        if let Err(e) = sqlx::query("ALTER TABLE tasks ADD COLUMN work_duration_minutes INTEGER")
            .execute(pool)
            .await
        {
            log_migration_err(
                "ALTER TABLE tasks ADD COLUMN work_duration_minutes INTEGER",
                e,
            );
        }
    ...
    if !applied_set.contains(&10) {
        if let Err(e) = sqlx::query("ALTER TABLE users ADD COLUMN password_changed_at TEXT")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE users ADD COLUMN password_changed_at TEXT", e);
        }
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (10, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&11) {
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (11, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&12) {
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (12, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&13) {
        ...
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (13, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&14) {
        if let Err(e) = sqlx::query("ALTER TABLE tasks ADD COLUMN estimate_optimistic REAL")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE tasks ADD COLUMN estimate_optimistic", e);
        }
        if let Err(e) = sqlx::query("ALTER TABLE tasks ADD COLUMN estimate_pessimistic REAL")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE tasks ADD COLUMN estimate_pessimistic", e);
        }
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (14, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&15) {
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (15, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&16) {
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (16, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&17) {
        if let Err(e) = sqlx::query("ALTER TABLE user_configs ADD COLUMN theme TEXT")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE user_configs ADD COLUMN theme", e);
        }
        if let Err(e) =
            sqlx::query("ALTER TABLE user_configs ADD COLUMN notify_desktop INTEGER DEFAULT 1")
                .execute(pool)
                .await
        {
            log_migration_err("ALTER TABLE user_configs ADD COLUMN notify_desktop", e);
        }
        if let Err(e) =
            sqlx::query("ALTER TABLE user_configs ADD COLUMN notify_sound INTEGER DEFAULT 1")
                .execute(pool)
                .await
        {
            log_migration_err("ALTER TABLE user_configs ADD COLUMN notify_sound", e);
        }
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (17, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&18) {
        ...
        .ok();
        sqlx::query(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_custom_statuses_name ON custom_statuses(name)",
        )
        .execute(pool)
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (18, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&19) {
        ...
        .ok();
        sqlx::query(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_custom_fields_name ON custom_fields(name)",
        )
        .execute(pool)
        .await
        .ok();
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (19, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&20) {
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (20, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&21) {
        if let Err(e) = sqlx::query("ALTER TABLE users ADD COLUMN email TEXT")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE users ADD COLUMN email", e);
        }
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (21, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&22) {
        ...
        .execute(pool)
        .await?;
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (22, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if applied_set.contains(&8) {
        let fts_exists = sqlx::query("SELECT 1 FROM tasks_fts LIMIT 0")
            .execute(pool)
            .await
            .is_ok();
    ...
    if !applied_set.contains(&23) {
        ...
        .execute(pool)
        .await?;
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (23, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&24) {
        if let Err(e) =
            sqlx::query("ALTER TABLE tasks ADD COLUMN updated_by INTEGER REFERENCES users(id)")
                .execute(pool)
                .await
        {
            log_migration_err("ALTER TABLE tasks ADD COLUMN updated_by", e);
        }
        if let Err(e) = sqlx::query("ALTER TABLE users ADD COLUMN timezone TEXT")
            .execute(pool)
            .await
        {
            log_migration_err("ALTER TABLE users ADD COLUMN timezone", e);
        }
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (24, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&25) {
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (25, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&26) {
        ...
        .await
        .ok();
        sqlx::query("CREATE UNIQUE INDEX IF NOT EXISTS idx_projects_key ON projects(key)")
            .execute(pool)
            .await
            .ok();
        ...
        if let Err(e) =
            sqlx::query("ALTER TABLE tasks ADD COLUMN project_id INTEGER REFERENCES projects(id)")
                .execute(pool)
                .await
        {
            log_migration_err("ALTER TABLE tasks ADD COLUMN project_id", e);
        }
        if let Err(e) =
            sqlx::query("ALTER TABLE sprints ADD COLUMN project_id INTEGER REFERENCES projects(id)")
                .execute(pool)
                .await
        {
            log_migration_err("ALTER TABLE sprints ADD COLUMN project_id", e);
        }
        if let Err(e) =
            sqlx::query("ALTER TABLE rooms ADD COLUMN project_id INTEGER REFERENCES projects(id)")
                .execute(pool)
                .await
        {
            log_migration_err("ALTER TABLE rooms ADD COLUMN project_id", e);
        }
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_tasks_project_id ON tasks(project_id)")
            .execute(pool)
            .await
            .ok();
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_sprints_project_id ON sprints(project_id)")
            .execute(pool)
            .await
            .ok();
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_rooms_project_id ON rooms(project_id)")
            .execute(pool)
            .await
            .ok();
        ...
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (26, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&27) {
        ...
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (27, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
    ...
    if !applied_set.contains(&28) {
        ...
        .await
        .ok();
        sqlx::query("INSERT OR IGNORE INTO schema_migrations (version, applied_at) VALUES (28, ?)")
            .bind(now_str())
            .execute(pool)
            .await
            .ok();
