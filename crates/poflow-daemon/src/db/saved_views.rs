...
pub async fn list_saved_views(pool: &Pool, user_id: i64) -> Result<Vec<SavedView>> {
    Ok(sqlx::query_as::<_, SavedView>(
        "SELECT * FROM saved_views WHERE user_id = ? ORDER BY updated_at DESC",
    )
    .bind(user_id)
    .fetch_all(pool)
    .await?)
...
pub async fn create_saved_view(
    ...
    Ok(
        sqlx::query_as::<_, SavedView>("SELECT * FROM saved_views WHERE id = ?")
            .bind(id)
            .fetch_one(pool)
            .await?,
    )
...
pub async fn update_saved_view(
    ...
    let r = sqlx::query(
        "UPDATE saved_views SET name = ?, filters = ?, updated_at = ? WHERE id = ? AND user_id = ?",
    )
    .bind(name)
    .bind(filters)
    .bind(now_str())
