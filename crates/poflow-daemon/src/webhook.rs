...
async fn is_safe_url(url: &str) -> Option<(String, std::net::SocketAddr)> {
    ...
    if let Ok(ip) = host.parse::<IpAddr>() {
        return if is_private_ip(&ip) {
            ...
            Some((url.to_string(), std::net::SocketAddr::new(ip, port)))
        };
    }
    // DNS resolution check — resolve once and pin IP
    match tokio::net::lookup_host(format!("{}:{}", host, port)).await {
        Ok(mut addrs) => addrs
            .find(|a| !is_private_ip(&a.ip()))
    ...
    tokio::spawn(async move {
        ...
        for hook in hooks {
            let Some((safe_url, pinned_addr)) = is_safe_url(&hook.url).await else {
                tracing::warn!(
                    "Webhook {} blocked: resolves to private/loopback IP",
                    hook.url
                );
                continue;
            };
            ...
            let pinned_client = {
                let host = url::Url::parse(&hook.url)
                    .ok()
            ...
            loop {
                ...
                let mut retry_req = pinned_client
                    .post(&safe_url)
                    .header("content-type", "application/json")
                    .header("x-poflow-event", &event)
                    .body(body_str.clone());
                if let Some(ref sig) = signature {
                    retry_req = retry_req.header("x-poflow-signature", sig.as_str());
                }
                ...
                let jitter_ms = if getrandom::fill(&mut jitter_buf).is_ok() {
                    jitter_buf[0] as u64 * 4
                } else {
                    0
                };
