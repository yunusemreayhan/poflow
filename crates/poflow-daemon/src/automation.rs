...
/// Run automation rules triggered by task creation.
pub async fn run_task_created(pool: &Pool, task_id: i64) {
    let rules: Vec<(i64, i64, String, String)> = match sqlx::query_as(
        "SELECT id, user_id, condition_json, action_json FROM automation_rules WHERE trigger_event = 'task.created' AND enabled = 1"
    ).fetch_all(pool).await {
        Ok(r) => r,
        Err(_) => return,
    };
...
/// Run automation rules triggered by task assignment.
pub async fn run_task_assigned(pool: &Pool, task_id: i64, assignee: &str) {
    let rules: Vec<(i64, i64, String, String)> = match sqlx::query_as(
        "SELECT id, user_id, condition_json, action_json FROM automation_rules WHERE trigger_event = 'task.assigned' AND enabled = 1"
    ).fetch_all(pool).await {
        Ok(r) => r,
        Err(_) => return,
    };
...
/// Run automation rules triggered by priority change.
pub async fn run_priority_changed(pool: &Pool, task_id: i64, old_priority: i64, new_priority: i64) {
    let rules: Vec<(i64, i64, String, String)> = match sqlx::query_as(
        "SELECT id, user_id, condition_json, action_json FROM automation_rules WHERE trigger_event = 'task.priority_changed' AND enabled = 1"
    ).fetch_all(pool).await {
        Ok(r) => r,
        Err(_) => return,
    };
