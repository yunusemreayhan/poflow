...
async fn test_delete_comment_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"alice2","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_threaded_comments() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/comments", tid),
            &tok,
            Some(json!({"content":"root comment"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/comments", tid),
            &tok,
            Some(json!({"content":"reply","parent_id":root_id})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let reply_comment = comments
        ...
        .unwrap()
        .iter()
        .find(|c| c["parent_id"].as_i64() == Some(root_id))
        .unwrap();
