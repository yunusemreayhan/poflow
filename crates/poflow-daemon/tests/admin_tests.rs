...
async fn test_non_root_cannot_admin() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"bob","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_delete_user_cascade() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"delme","password":"Pass1234"})),
        ))
        .await
        .unwrap();
...
async fn test_delete_user_preserves_comments_and_burns() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &tok2,
            Some(json!({"content":"Important context"})),
        ))
        .await
        .unwrap();
    ...
    assert!(
        comments.iter().any(|c| c["content"] == "Important context"),
        "Comment should survive user deletion"
    );
    let resp = app
        .clone()
        .oneshot(auth_req(
    ...
    assert!(
        !burns.as_array().unwrap().is_empty(),
        "Burns should survive user deletion"
    );
}
...
async fn test_users_list_public() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/users", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let users = body_json(resp).await;
...
async fn test_backup_root_only() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/admin/backup", &user_tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/admin/backup", &tok, None))
        .await
        .unwrap();
...
async fn test_backup_list() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/admin/backups", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let resp = app
...
async fn test_internal_error_no_leak() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/999999", &tok, None))
        .await
        .unwrap();
    if resp.status() == 500 {
        ...
        assert!(
            !msg.contains("sqlx"),
            "Error message should not contain sqlx details"
        );
        assert!(
            !msg.contains("SELECT"),
        ...
        assert!(
            !msg.contains("tasks"),
            "Error message should not contain table names"
        );
...
async fn test_admin_can_create_custom_statuses_and_fields() {
    ...
    app.clone()
        .oneshot(auth_req(
            "PUT",
            &format!("/api/admin/users/{}/role", admin_uid),
            &tok,
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"admcf","password":"AdmCF1111"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/statuses",
            &admin_tok,
            Some(json!({"name":"admin_review","category":"in_progress"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/fields",
            &admin_tok,
            Some(json!({"name":"admin_field","field_type":"text"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_admin_can_manage_labels() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"admlab","password":"AdmLab111"})),
        ))
        .await
        .unwrap();
    let admin_tok = body_json(resp).await["token"].as_str().unwrap().to_string();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/labels",
            &admin_tok,
            Some(json!({"name":"admin_label","color":"#ff0000"})),
        ))
        .await
        .unwrap();
...
async fn test_admin_can_manage_others_tasks() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"admtask_admin","password":"AdmTA1111"})),
        ))
        .await
        .unwrap();
    let admin_tok = body_json(resp).await["token"].as_str().unwrap().to_string();
...
async fn test_admin_cannot_manage_users() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"admnouser","password":"AdmNU1111"})),
        ))
        .await
        .unwrap();
    let admin_tok = body_json(resp).await["token"].as_str().unwrap().to_string();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/admin/backup", &admin_tok, None))
        .await
        .unwrap();
    assert_eq!(
        resp.status(),
        403,
        "Admin should NOT be able to create backups"
    );
}
