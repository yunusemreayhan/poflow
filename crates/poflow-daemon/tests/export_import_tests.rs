...
async fn test_export_tasks_csv() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"ExportMe"})),
        ))
        .await
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/export/tasks?format=csv", &tok, None))
        .await
        .unwrap();
...
async fn test_export_tasks_json() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Export Me"})),
        ))
        .await
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/export/tasks?format=json", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let ct = resp
...
async fn test_export_sessions_csv() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/export/sessions?format=csv",
            &tok,
            None,
        ))
        .await
...
async fn test_export_sessions_date_range() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/export/sessions?format=json&from=2020-01-01&to=2020-12-31",
            &token,
            None,
        ))
        .await
...
async fn test_csv_import_malformed() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/import/tasks",
            &tok,
            Some(json!({"csv":"title,priority,estimated,project,extra1,extra2\nT,1,2,proj,x,y\n"})),
        ))
        .await
        .unwrap();
    let body = body_json(resp).await;
...
async fn test_csv_export() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"CSV1","project":"P1"})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"CSV2","project":"P1"})),
        ))
        .await
        .unwrap();
...
async fn test_ical_export() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"ical task","due_date":"2026-06-15"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/export/ical", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let bytes = resp.into_body().collect().await.unwrap().to_bytes();
...
async fn test_ical_excludes_deleted() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"deleted ical","due_date":"2026-12-25"})),
        ))
        .await
        .unwrap();
...
async fn test_project_export_includes_all_data() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"ExportTask","project":"ExportProj"})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"content":"Test comment"})),
        ))
        .await
        .unwrap();
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/labels",
            &tok,
            Some(json!({"name":"export_label","color":"#ff0000"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/export/project?project=ExportProj",
            &tok,
            None,
        ))
        .await
...
async fn test_project_import_roundtrip() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"ImportMe","project":"RoundTrip","priority":4})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/comments", tid),
            &tok,
            Some(json!({"content":"Round trip comment"})),
        ))
        .await
        .unwrap();
    app.clone()
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/export/project?project=RoundTrip",
            &tok,
            None,
        ))
        .await
...
async fn test_project_export_empty_project() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/export/project?project=NonExistentProject",
            &tok,
            None,
        ))
        .await
