...
async fn test_update_task() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", id),
            &tok,
            Some(json!({"title":"New","status":"in_progress","priority":5,"estimated_hours":8.0})),
        ))
        .await
        .unwrap();
...
async fn test_subtask_cascade_delete() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Child","parent_id":pid})),
        ))
        .await
        .unwrap();
...
async fn test_delete_task_cascades_burns_and_sprint_tasks() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"task_id":tid,"points":5.0})),
        ))
        .await
        .unwrap();
...
async fn test_optimistic_locking_task() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", id),
            &tok,
            Some(json!({"title":"T2","expected_updated_at":updated_at})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", id),
            &tok,
            Some(json!({"title":"T3","expected_updated_at":updated_at})),
        ))
        .await
        .unwrap();
...
async fn test_concurrent_task_updates() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", id),
            &tok,
            Some(json!({"title":"Updated1","expected_updated_at":updated_at})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", id),
            &tok,
            Some(json!({"title":"Updated2","expected_updated_at":updated_at})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 409);
...
async fn test_tasks_full_etag() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/full", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let etag = resp
...
async fn test_task_detail_with_children() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Child1","parent_id":pid})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Child2","parent_id":pid})),
        ))
        .await
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req("GET", &format!("/api/tasks/{}", pid), &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_task_search_filter() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Backend API","project":"backend"})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Frontend UI","project":"frontend"})),
        ))
        .await
        .unwrap();
    // Search by text
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks?search=backend", &tok, None))
        .await
        .unwrap();
    let tasks = body_json(resp).await;
    assert!(tasks.as_array().unwrap().iter().all(|t| t["title"]
        .as_str()
        .unwrap()
        .to_lowercase()
        .contains("backend")
        || t["project"]
            .as_str()
            .is_some_and(|p| p.to_lowercase().contains("backend"))));
    // Filter by project
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks?project=frontend", &tok, None))
        .await
        .unwrap();
    let tasks = body_json(resp).await;
...
async fn test_task_pagination() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks?page=1&per_page=2", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_task_sprints_endpoint() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/task-sprints", &tok, None))
        .await
        .unwrap();
...
async fn test_task_sprints_with_data() {
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/sprints/{}/tasks", sprint["id"]),
            &tok,
...
async fn test_task_negative_estimated_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T","estimated_hours":-1.0})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_task_invalid_due_date_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T","due_date":"2024/01/01"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T","due_date":"not-a-date"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T","due_date":"2026-12-31"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_circular_parent_id_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"B","parent_id":a_id})),
        ))
        .await
        .unwrap();
...
async fn test_task_duplicate() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/duplicate", oid),
            &tok,
            None,
        ))
        .await
...
async fn test_soft_delete_cascade_restore() {
    ...
    assert!(!tasks
        ...
        .unwrap()
        .iter()
        .any(|t| t["id"] == pid || t["id"] == cid));

    // Both should appear in trash
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/trash", &tok, None))
        .await
        .unwrap();
...
async fn test_task_work_duration() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"work_duration_minutes": 45})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"work_duration_minutes": 999})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_task_update_conflict_detection() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"title":"Updated","expected_updated_at": updated_at})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"title":"Stale","expected_updated_at": updated_at})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 409);
...
async fn test_tasks_full_returns_pagination_headers() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/tasks/full?page=1&per_page=10",
            &tok,
            None,
        ))
        .await
    ...
    assert_eq!(
        resp.headers().get("x-per-page").unwrap().to_str().unwrap(),
        "10"
    );
}
...
async fn test_tasks_full_project_filter() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"ProjA","project":"alpha"})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"ProjB","project":"beta"})),
        ))
        .await
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/full?project=alpha", &tok, None))
        .await
        .unwrap();
...
async fn test_tasks_full_pagination() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/tasks/full?per_page=2&page=1",
            &tok,
            None,
        ))
        .await
...
async fn test_tasks_full_scoped_joins() {
    ...
    for i in 0..3 {
        let resp = app
            ...
            .oneshot(auth_req(
                ...
                "/api/tasks",
                &tok,
                Some(json!({"title": format!("Scoped{}", i)})),
            ))
            .await
            .unwrap();
        let j = body_json(resp).await;
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/assignees", ids[0]),
            &tok,
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/labels",
            &tok,
            Some(json!({"name": "scoped-lbl", "color": "#ff0000"})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            "PUT",
            &format!("/api/tasks/{}/labels/{}", ids[2], label_id),
            &tok,
            None,
        ))
        .await
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/tasks/full?per_page=1&page=1",
            &tok,
            None,
        ))
        .await
    ...
    for a in j["assignees"].as_array().unwrap() {
        assert_eq!(
            a["task_id"].as_i64().unwrap(),
            page_id,
            "assignee leaked from another page"
        );
    }
    // Labels should only contain entries for the page task
    for l in j["labels"].as_array().unwrap() {
        assert_eq!(
            l["task_id"].as_i64().unwrap(),
            page_id,
            "label leaked from another page"
        );
    }
}
...
async fn test_fts5_search() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Quantum physics research"})),
        ))
        .await
        .unwrap();
    app.clone()
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks?search=quantum", &tok, None))
        .await
        .unwrap();
    let tasks = body_json(resp).await;
    ...
    assert!(arr
        .iter()
        .any(|t| t["title"].as_str().unwrap().contains("Quantum")));
    assert!(!arr
        .iter()
        .any(|t| t["title"].as_str().unwrap().contains("groceries")));
}
...
async fn test_fts5_snippet_xss_sanitized() {
    ...
    for r in arr {
        ...
        assert!(
            ...
            "Script tags should not appear in search results: {}",
            title
        );
    }
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/search?q=xsstest123", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let j = body_json(resp).await;
    for t in j["tasks"].as_array().unwrap() {
        ...
        assert!(
            ...
            "Script tags should not appear in global search snippets: {}",
            snippet
        );
    }
}
...
async fn test_advanced_search_like_escape() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T1","project":"alpha_beta"})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T2","project":"alphaXbeta"})),
        ))
        .await
        .unwrap();
    ...
    assert!(arr
        .iter()
        .any(|t| t["project"].as_str().unwrap_or("") == "alpha_beta"));
    assert!(
        !arr.iter()
            .any(|t| t["project"].as_str().unwrap_or("") == "alphaXbeta"),
    ...
    for from in &statuses {
        for to in &statuses {
            ...
            if *from != "backlog" {
                ...
                assert_eq!(
                    sc, 200,
                    "Failed to set initial status to '{}' for task {}",
                    from, tid
                );
...
async fn test_invalid_status_rejected() {
    ...
    for bad in &[
        ...
        assert_eq!(
            sc, 400,
            "Status '{}' should be rejected but got {}",
            bad, sc
        );
    }
}
    ...
    for status in &statuses {
        ...
        let resp = app
            ...
            .oneshot(auth_req(
                ...
                &format!("/api/tasks/{}/assignees", tid),
                &tok,
                Some(json!({"username": "assignee_tester"})),
            ))
            .await
            .unwrap();
        assert_eq!(
        ...
        assert!(
            assignees.contains(&"assignee_tester".to_string()),
            "Assignee not found in '{}' status",
            status
        );
        ...
        let resp = app
            ...
            .oneshot(auth_req(
                "DELETE",
                &format!("/api/tasks/{}/assignees/assignee_tester", tid),
                &tok,
                None,
            ))
            .await
        ...
        assert!(
            !assignees.contains(&"assignee_tester".to_string()),
            "Assignee still present after unassign in '{}' status",
            status
        );
    }
}
...
async fn test_status_change_preserves_assignees() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/assignees", tid),
            &tok,
            Some(json!({"username": "persist_user"})),
        ))
        .await
        .unwrap();
    ...
    for status in &transitions {
        ...
        let assignees = get_assignees(&app, &tok, tid).await;
        assert!(
            assignees.contains(&"persist_user".to_string()),
            "Assignee lost after transition to '{}'",
            status
        );
    }
}
...
async fn test_bulk_status_change_mixed() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"task_ids": [t1, t2, t3, t4], "status": "completed"})),
        ))
        .await
        .unwrap();
    ...
    for tid in [t1, t2, t3, t4] {
        ...
        assert_eq!(
            ...
            "Task {} not completed after bulk update",
            tid
        );
    }
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &tok,
            Some(json!({"task_ids": [t1, t2, t3, t4], "status": "backlog"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status().as_u16(), 204);
...
async fn test_bulk_status_invalid_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &tok,
            Some(json!({"task_ids": [t1], "status": "invalid_status"})),
        ))
        .await
        .unwrap();
...
async fn test_bulk_status_empty_ids() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &tok,
            Some(json!({"task_ids": [], "status": "completed"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status().as_u16(), 204);
...
async fn test_double_assign_same_user() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/assignees", tid),
            &tok,
            Some(json!({"username": "double_assign_user"})),
        ))
        .await
        .unwrap();
...
async fn test_assign_nonexistent_user() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/assignees", tid),
            &tok,
            Some(json!({"username": "nonexistent_user_xyz"})),
        ))
        .await
        .unwrap();
...
async fn test_unassign_nonexistent_user() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/assignees/nonexistent_user_xyz", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_assign_to_deleted_task() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/assignees", tid),
            &tok,
            Some(json!({"username": "assign_deleted_user"})),
        ))
        .await
        .unwrap();
    ...
    assert!(
        sc == 200 || sc == 404,
        "Unexpected status {} for assign to deleted task",
        sc
    );
}
...
async fn test_multiple_assignees() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/assignees/multi_a2", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_non_owner_cannot_assign() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/assignees", tid),
            &user_tok,
            Some(json!({"username": "target_assignee"})),
        ))
        .await
        .unwrap();
...
async fn test_assignee_can_self_unassign() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/assignees", tid),
            &tok,
            Some(json!({"username": "self_unassigner"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/assignees/self_unassigner", tid),
            &user_tok,
            None,
        ))
        .await
...
async fn test_bulk_status_preserves_assignees() {
    ...
    for tid in [t1, t2] {
        app.clone()
            .oneshot(auth_req(
                ...
                &format!("/api/tasks/{}/assignees", tid),
                &tok,
                Some(json!({"username": "bulk_assignee"})),
            ))
            .await
            .unwrap();
    }
    ...
    for tid in [t1, t2] {
        let assignees = get_assignees(&app, &tok, tid).await;
        assert!(
            assignees.contains(&"bulk_assignee".to_string()),
            "Assignee lost after bulk status change on task {}",
            tid
        );
    }
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &tok,
            Some(json!({"task_ids": [t1, t2], "status": "backlog"})),
        ))
        .await
        .unwrap();
    ...
    for tid in [t1, t2] {
        ...
        assert!(
            assignees.contains(&"bulk_assignee".to_string()),
            "Assignee lost after bulk reopen on task {}",
            tid
        );
    }
}
...
async fn test_bulk_assign() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"task_ids":[t1,t2],"username":"bulkdev"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 204);
...
async fn test_bulk_sprint_move() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            "/api/tasks/bulk-sprint",
            &tok,
            Some(json!({"task_ids":[t1,t2],"sprint_id":sid})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 204);
...
async fn test_bulk_assign_nonexistent_user_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-assign",
            &tok,
            Some(json!({"task_ids":[tid],"username":"nonexistent_user_xyz"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 404);
...
async fn test_advanced_search_multi_status() {
    ...
    assert!(statuses
        .iter()
        .all(|s| *s == "in_progress" || *s == "completed"));
}
...
async fn test_advanced_search_combined_filters() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"CF1","priority":5,"project":"SearchProj"})),
        ))
        .await
        .unwrap();
    let t1 = body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"CF2","priority":1,"project":"SearchProj"})),
        ))
        .await
        .unwrap();
...
async fn test_advanced_search_custom_field_filter() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/fields",
            &tok,
            Some(json!({"name":"env","field_type":"text"})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            "PUT",
            &format!("/api/tasks/{}/fields/{}", t1, fid),
            &tok,
            Some(json!({"value":"production"})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            "PUT",
            &format!("/api/tasks/{}/fields/{}", t2, fid),
            &tok,
            Some(json!({"value":"development"})),
        ))
        .await
        .unwrap();
...
async fn test_advanced_search_with_sort() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Sort1","priority":1})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Sort5","priority":5})),
        ))
        .await
        .unwrap();
...
async fn test_advanced_search_title_contains() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"UniqueNeedle12345"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    assert!(tasks
        ...
        .unwrap()
        .iter()
        .any(|t| t["title"].as_str().unwrap().contains("Needle")));
}
...
async fn test_checklist_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/checklist", tid),
            &tok,
            Some(json!({"title":"Step 1"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_checklist_cascade_on_task_delete() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/checklist", tid),
            &tok,
            Some(json!({"title":"Will be deleted"})),
        ))
        .await
        .unwrap();
...
async fn test_task_links_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"link_type":"pr","url":"https://github.com/test/pr/1","title":"Fix bug"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_task_link_url_too_long() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/links", tid),
            &tok,
            Some(json!({"link_type":"pr","url":long_url,"title":"x"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_pert_estimates_null_clear() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"estimate_optimistic": 5.0})),
        ))
        .await
        .unwrap();
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"estimate_optimistic": null})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_pert_copied_on_duplicate() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"estimate_optimistic": 1.0, "estimate_pessimistic": 8.0})),
        ))
        .await
        .unwrap();
    // Duplicate
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/duplicate", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_global_search() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"UniqueSearchTarget42"})),
        ))
        .await
        .unwrap();
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/search?q=UniqueSearchTarget42",
            &tok,
            None,
        ))
        .await
    ...
    let results = body_json(resp).await;
    assert!(
        !results["tasks"].as_array().unwrap().is_empty(),
...
async fn test_global_search_comments() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"content":"UniqueCommentXyz99"})),
        ))
        .await
        .unwrap();
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/search?q=UniqueCommentXyz99",
            &tok,
            None,
        ))
        .await
    ...
    let results = body_json(resp).await;
    assert!(
        !results["comments"].as_array().unwrap().is_empty(),
...
async fn test_global_search_empty_query() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/search?q=", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_reorder_persists_sort_order() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks/reorder",
            &tok,
            Some(json!({"orders":[[c, 0],[a, 1000],[b, 2000]]})),
        ))
        .await
        .unwrap();
...
async fn test_archive_and_unarchive_task() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/archived", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks?status=backlog", &tok, None))
        .await
        .unwrap();
    let tasks = body_json(resp).await;
    assert!(
        !tasks.as_array().unwrap().iter().any(|t| t["id"] == tid),
        "Archived task should not appear when filtering by backlog"
    );
    // Unarchive
    let resp = app
        .clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/unarchive", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_unarchive_non_archived_rejected() {
    ...
    assert_eq!(
        resp.status(),
        400,
        "Should reject unarchive on non-archived task"
    );
}
...
async fn test_archived_list_user_scoped() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/archived", &user_tok, None))
        .await
        .unwrap();
    let archived = body_json(resp).await;
    assert!(!archived.as_array().unwrap().iter().any(|t| t["id"] == tid));
...
async fn test_purge_task() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/permanent", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_search_tasks() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Searchable Widget"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    assert!(j
        ...
        .unwrap()
        .iter()
        .any(|t| t["title"].as_str().unwrap().contains("Searchable")));
}
...
async fn test_search_tasks_empty_query() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/search?q=", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let j = body_json(resp).await;
