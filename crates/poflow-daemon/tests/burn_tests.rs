...
async fn test_burn_log_and_cancel() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"task_id":tid,"points":5.0,"hours":2.0,"note":"Did stuff"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/sprints/{}/burn-summary", sid),
            &tok,
            None,
        ))
        .await
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/sprints/{}/burns/{}", sid, bid),
            &tok,
            None,
        ))
        .await
...
async fn test_burn_multi_user_summary() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/sprints/{}/burn", sid),
            &tok,
            Some(json!({"task_id":tid,"points":3.0})),
        ))
        .await
        .unwrap();
    app.clone()
...
async fn test_burn_log_requires_authorization() {
    ...
    assert_eq!(
        resp.status(),
        403,
        "Unrelated user should not be able to log burns"
    );
    // Sprint owner can
    let resp = app
        .clone()
...
async fn test_cancel_burn_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"burnuser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_cancel_burn_validates_sprint() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/sprints/{}/burn", sid),
            &tok,
            Some(json!({"task_id":tid,"points":1.0,"hours":0.5})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/sprints/99999/burns/{}", burn_id),
            &tok,
            None,
        ))
        .await
...
async fn test_time_reports_auto_assign() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/time", tid),
            &tok,
            Some(json!({"hours":2.5,"description":"work"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/tasks/{}/burn-total", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_time_report_links_to_active_sprint() {
    ...
    assert_eq!(
        burn["sprint_id"], sid,
        "time report should link to active sprint"
    );
...
async fn test_burn_totals_endpoint() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/burn-totals", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
}
...
async fn test_time_tracking_report_json() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"TT","project":"Proj1"})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/reports/time-tracking", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let ct = resp
...
async fn test_time_tracking_report_csv() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/reports/time-tracking?format=csv",
            &tok,
            None,
        ))
        .await
...
async fn test_task_burn_users() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/tasks/{}/burn-users", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_task_time_summary() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/tasks/{}/time-summary", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_active_timers() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/timer/active", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let j = body_json(resp).await;
