...
pub fn auth_req(method: &str, uri: &str, token: &str, body: Option<Value>) -> Request<Body> {
    static AUTH_COUNTER: std::sync::atomic::AtomicU64 =
        std::sync::atomic::AtomicU64::new(5_000_000);
...
pub async fn register_user(app: &axum::Router, username: &str) -> String {
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username": username, "password": "Pass1234"})),
        ))
        .await
        .unwrap();
    assert!(
...
#[allow(dead_code)]
pub async fn register_user_full(
    app: &axum::Router,
    username: &str,
    password: &str,
) -> (String, i64) {
