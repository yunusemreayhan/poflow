...
mod common;
use common::{app, auth_req, body_json, json_req, login_root, register_user, register_user_full};
...
async fn test_history_empty() {
    ...
    let resp = app
        .oneshot(auth_req("GET", "/api/history", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_history_date_range() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/history?from=2026-01-01&to=2026-12-31",
            &tok,
            None,
        ))
        .await
...
async fn test_stats_endpoint() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/stats?from=2026-01-01&to=2026-12-31",
            &tok,
            None,
        ))
        .await
...
async fn test_profile_update() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"profuser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &new_tok,
            Some(json!({"password":"NewPass12","current_password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"profuser2","password":"NewPass12"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"profuser2","password":"Pass1234"})),
        ))
        .await
        .unwrap();
...
async fn test_profile_password_change() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &tok,
            Some(json!({"password":"NewPass123","current_password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"pwChangeUser","password":"NewPass123"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"pwChangeUser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 401);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &tok2,
            Some(json!({"password":"NewPass123","current_password":"WrongPass1"})),
        ))
        .await
        .unwrap();
...
async fn test_profile_weak_password_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &tok,
            Some(json!({"password":"NoDigitHere"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_password_change_requires_current() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &tok,
            Some(json!({"password":"NewPass99","current_password":"WrongPass1"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &tok,
            Some(json!({"password":"NewPass99","current_password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_velocity() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/sprints/velocity", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_velocity_with_limit() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/sprints/velocity?sprints=5",
            &tok,
            None,
        ))
        .await
...
async fn test_global_burndown() {
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/sprints/{}/snapshot", sid),
            &tok,
            None,
        ))
        .await
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/sprints/burndown", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_due_date_reminder_query() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Due soon","due_date":&tomorrow})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Not due","due_date":"2099-12-31"})),
        ))
        .await
        .unwrap();
    assert!(resp.status().is_success());
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Done task","due_date":&tomorrow})),
        ))
        .await
        .unwrap();
    assert!(resp.status().is_success());
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks", &tok, None))
        .await
        .unwrap();
    let tasks = body_json(resp).await;
    let due_tasks: Vec<&Value> = tasks
        .as_array()
        .unwrap()
        .iter()
        .filter(|t| {
            t["due_date"]
                .as_str()
                .is_some_and(|d| d <= day_after.as_str())
...
async fn test_assignees() {
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/assignees", tid),
            &tok,
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/assignees/root", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_remove_assignee_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"assignuser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_assignee_add_list_remove() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/assignees", tid),
            &tok,
            Some(json!({"username":"assigneeUser"})),
        ))
        .await
        .unwrap();
    assert!(resp.status().is_success());
    ...
    assert!(assignees
        .as_array()
        .unwrap()
        .contains(&json!("assigneeUser")));
    let resp = app
        .clone()
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/assignees/assigneeUser", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_all_assignees_endpoint() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/assignees", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
}
...
async fn test_notification_prefs_crud() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/profile/notifications", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!([{"event_type":"task_assigned","enabled":false}])),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let ta = prefs
        ...
        .unwrap()
        .iter()
        .find(|p| p["event_type"] == "task_assigned")
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile/notifications",
            &tok,
            Some(json!([{"event_type":"bogus","enabled":true}])),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_bulk_status_change() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &token,
            Some(json!({"task_ids":[id1,id2],"status":"done"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 204);
...
async fn test_bulk_status_invalid() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &token,
            Some(json!({"task_ids":[999],"status":"invalid"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_bulk_status_ownership_isolation() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"user2","password":"Pass1234!"})),
        ))
        .await
        .unwrap();
    let tok2 = body_json(resp).await["token"].as_str().unwrap().to_string();
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &tok2,
            Some(json!({"task_ids":[tid],"status":"completed"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
...
async fn test_bulk_status_mixed_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"bulkuser","password":"Password1!"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    let root_tid = body_json(
        app.clone()
            .oneshot(auth_req(
                "POST",
                "/api/tasks",
                &tok,
                Some(json!({"title":"RootOwned"})),
            ))
            .await
            .unwrap(),
    )
    .await["id"]
        .as_i64()
        .unwrap();
    let user_tid = body_json(
        app.clone()
            .oneshot(auth_req(
                "POST",
                "/api/tasks",
                &user_tok,
                Some(json!({"title":"UserOwned"})),
            ))
            .await
            .unwrap(),
    )
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &user_tok,
            Some(json!({"task_ids":[root_tid, user_tid],"status":"active"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
...
async fn test_bulk_status_update() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &tok,
            Some(json!({"task_ids":[t1, t2],"status":"completed"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 204);
    let resp = app
        .clone()
        .oneshot(auth_req("GET", &format!("/api/tasks/{}", t1), &tok, None))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks/bulk-status",
            &tok,
            Some(json!({"task_ids":[t1],"status":"invalid"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_user_hours_report() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/reports/user-hours", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/reports/user-hours?from=garbage",
            &tok,
            None,
        ))
        .await
...
async fn test_user_timezone_update() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &tok,
            Some(json!({"timezone":"Europe/Stockholm"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_watcher_notified_on_comment() {
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/comments", tid),
            &root_tok,
            Some(json!({"content":"Hello watchers"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/notifications", &alice_tok, None))
        .await
        .unwrap();
...
async fn test_watcher_not_notified_own_comment() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/comments", tid),
            &root_tok,
            Some(json!({"content":"My own comment"})),
        ))
        .await
        .unwrap();
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/notifications", &root_tok, None))
        .await
        .unwrap();
    let notifs = body_json(resp).await;
    let found = notifs
    ...
    assert!(
        !found,
        "Commenter should NOT receive own comment notification"
    );
}
...
async fn test_watcher_notified_on_status_change() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/notifications", &bob_tok, None))
        .await
        .unwrap();
    let notifs = body_json(resp).await;
    let found = notifs
        ...
        .unwrap()
        .iter()
        .any(|n| n["kind"] == "task_status_changed");
    assert!(
        found,
        "Watcher should receive task_status_changed notification"
    );
}
...
async fn test_notification_prefs_include_new_types() {
    ...
    let resp = app
        ...
        .unwrap();
    let prefs = body_json(resp).await;
    let types: Vec<&str> = prefs
        .as_array()
        .unwrap()
        .iter()
        .map(|p| p["event_type"].as_str().unwrap())
        .collect();
    assert!(
        types.contains(&"task_status_changed"),
        "Should include task_status_changed"
    );
...
async fn test_saved_views_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/views",
            &tok,
            Some(json!({"name":"My Filter","filters":{"status":"active","project":"alpha"}})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/views", &tok, None))
        .await
        .unwrap();
    ...
    assert!(views
        ...
        .unwrap()
        .iter()
        .any(|v| v["id"].as_i64() == Some(vid)));
    let resp = app
        .clone()
        .oneshot(auth_req(
            "PUT",
            &format!("/api/views/{}", vid),
            &tok,
            Some(json!({"name":"Updated","filters":{"status":"done"}})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_saved_views_user_isolation() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/views",
            &tok_a,
            Some(json!({"name":"A's view","filters":{}})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/views", &tok_b, None))
        .await
        .unwrap();
    let views = body_json(resp).await;
    assert!(!views
...
async fn test_projects_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/projects",
            &tok,
            Some(json!({"name":"Alpha Project","description":"Test project","key":"ALPHA"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/projects", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/projects/{}", pid),
            &tok,
            Some(json!({"name":"Alpha v2","status":"archived"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_projects_non_admin_rejected() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/projects", &user_tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
}
...
async fn test_task_with_project_id() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/projects",
            &tok,
            Some(json!({"name":"TaskProj","key":"TP"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Proj Task","project_id":pid})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_project_delete_unlinks_tasks() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/projects",
            &tok,
            Some(json!({"name":"DelProj","key":"DEL"})),
        ))
        .await
        .unwrap();
    let pid = body_json(resp).await["id"].as_i64().unwrap();
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Linked","project_id":pid})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", &format!("/api/tasks/{}", tid), &tok, None))
        .await
        .unwrap();
    let task = body_json(resp).await;
...
async fn test_transition_rules_crud() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/workflows/transitions",
            &token,
            Some(serde_json::json!({"from_status": "backlog", "to_status": "active"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            "DELETE",
            &format!("/api/workflows/transitions/{}", tid),
            &token,
            None,
        ))
        .await
...
async fn test_transition_rules_same_status_rejected() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/workflows/transitions",
            &token,
            Some(serde_json::json!({"from_status": "active", "to_status": "active"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_transition_enforcement_blocks_invalid() {
    ...
    app.clone()
        .oneshot(common::auth_req(
            ...
            "/api/workflows/transitions",
            &token,
            Some(serde_json::json!({"from_status": "active", "to_status": "completed"})),
        ))
        .await
        .unwrap();
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Trans test"})),
        ))
        .await
        .unwrap();
...
async fn test_transition_no_rules_allows_all() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Free trans"})),
        ))
        .await
        .unwrap();
    let task = common::body_json(resp).await;
...
async fn test_transition_enforcement_bulk_status() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Bulk1"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks/bulk-status",
            &token,
            Some(serde_json::json!({"task_ids": [t1], "status": "completed"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks/bulk-status",
            &token,
            Some(serde_json::json!({"task_ids": [t1], "status": "active"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 204);
...
async fn test_transition_project_specific_rules() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/projects",
            &token,
            Some(serde_json::json!({"name": "TransProj", "key": "TP"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Proj task", "project_id": proj_id})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "No proj task"})),
        ))
        .await
        .unwrap();
...
async fn test_automation_status_change_sets_priority() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Auto test", "priority": 3})),
        ))
        .await
        .unwrap();
    let task = common::body_json(resp).await;
...
async fn test_automation_all_subtasks_done() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Child", "parent_id": parent_id})),
        ))
        .await
        .unwrap();
...
async fn test_automation_task_created_trigger() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Auto created"})),
        ))
        .await
        .unwrap();
    let task_id = common::body_json(resp).await["id"].as_i64().unwrap();
...
async fn test_automation_task_assigned_trigger() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Assign test"})),
        ))
        .await
        .unwrap();
    let task_id = common::body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(common::auth_req(
            ...
            &token,
            Some(serde_json::json!({"username": "assignee1"})),
        ))
        .await
        .unwrap();
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
...
async fn test_automation_priority_changed_trigger() {
    ...
    let resp = app
        ...
        .oneshot(common::auth_req(
            ...
            "/api/tasks",
            &token,
            Some(serde_json::json!({"title": "Priority test", "priority": 3})),
        ))
        .await
        .unwrap();
    let task_id = common::body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(common::auth_req(
            ...
            &format!("/api/tasks/{}", task_id),
            &token,
            Some(serde_json::json!({"priority": 5})),
        ))
        .await
        .unwrap();
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
...
async fn test_session_note_update() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "PUT",
            &format!("/api/sessions/{}/note", sid),
            &tok,
            Some(json!({"note":"Updated note"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_task_watchers() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/watched", &tok, None))
        .await
        .unwrap();
...
async fn test_auto_archive() {
    ...
    app.clone()
        ...
        .await
        .unwrap();
    sqlx::query("UPDATE tasks SET updated_at = '2020-01-01T00:00:00' WHERE id = ?")
        .bind(tid)
        .execute(&pool)
        .await
        .unwrap();
...
async fn test_global_search_comment_isolation() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &root_tok,
            Some(json!({"title":"Root secret task"})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &root_tok,
            Some(json!({"content":"xyzzy_secret_keyword"})),
        ))
        .await
        .unwrap();
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/search?q=xyzzy_secret_keyword",
            &user_tok,
            None,
        ))
        .await
    ...
    assert!(
        comments.is_empty(),
        "Non-admin user should not see comments on other users' tasks"
    );
    let resp = app
        .clone()
        .oneshot(auth_req(
...
async fn test_achievements_check() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/achievements/check", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_achievements_list() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/achievements", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_activity_feed() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"feed task"})),
        ))
        .await
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/feed", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_activity_feed_includes_all_types() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/comments", tid),
            &tok,
            Some(json!({"content":"Feed comment"})),
        ))
        .await
        .unwrap();
    // Create sprint
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/sprints/{}/burn", sid),
            &tok,
            Some(json!({"task_id":tid,"hours":2.0})),
        ))
        .await
        .unwrap();
    // Get feed
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/feed?limit=50", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let types: Vec<&str> = items
        ...
        .collect();
    assert!(
        types.contains(&"audit"),
        "Feed should contain audit entries"
    );
    assert!(types.contains(&"comment"), "Feed should contain comments");
    assert!(
        types.contains(&"sprint"),
        "Feed should contain sprint events"
    );
...
async fn test_activity_feed_invalid_since() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/feed?since=garbage", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
}
...
async fn test_activity_feed_with_filters() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"feed filter task"})),
        ))
        .await
        .unwrap();
    // Filter by type
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/feed?types=audit&limit=5", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_automation_condition_filters() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"CondTask","priority":5})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        ...
        .unwrap();
    assert_eq!(
        body_json(resp).await["task"]["priority"],
        5,
        "Automation should NOT fire for backlog→completed"
    );
}
...
async fn test_automation_disabled_rule_does_not_fire() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"NoAuto","priority":5})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
...
async fn test_automation_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"autouser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_automation_rules_crud() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/automations", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "PUT",
            &format!("/api/automations/{}/toggle", rule_id),
            &tok,
            None,
        ))
        .await
...
async fn test_automation_status_change_fires() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"AutoTask","priority":5})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    assert_eq!(
        detail["task"]["priority"], 1,
        "Automation should have set priority to 1"
    );
}
...
async fn test_change_password() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/auth/password",
            &tok,
            Some(json!({"current_password":"OldPass123","new_password":"NewPass456"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 204);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"pwuser","password":"NewPass456"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"pwuser","password":"OldPass123"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 401);
...
async fn test_change_password_wrong_current() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/auth/password",
            &tok,
            Some(json!({"current_password":"WrongPass","new_password":"NewPass456"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 401);
...
async fn test_custom_field_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/fields",
            &tok,
            Some(json!({"name":"Sprint Goal","field_type":"text"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/fields", &tok, None))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "PUT",
            &format!("/api/fields/{}", fid),
            &tok,
            Some(json!({"name":"Goal","field_type":"text","required":true})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_custom_field_in_task_detail() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/fields",
            &tok,
            Some(json!({"name":"Priority Level","field_type":"select","options":["P0","P1","P2"]})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            "PUT",
            &format!("/api/tasks/{}/fields/{}", tid, fid),
            &tok,
            Some(json!({"value":"P0"})),
        ))
        .await
...
async fn test_custom_field_select_requires_options() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/fields",
            &tok,
            Some(json!({"name":"NoOpts","field_type":"select"})),
        ))
        .await
        .unwrap();
    assert_eq!(
        resp.status(),
        400,
        "Select field without options should be rejected"
    );
}
...
async fn test_custom_field_values_on_task() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/fields",
            &tok,
            Some(json!({"name":"Component","field_type":"text"})),
        ))
        .await
        .unwrap();
    let fid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        ...
        .unwrap();
    assert_eq!(
        body_json(resp).await.as_array().unwrap()[0]["value"],
        "Frontend"
    );
    // Delete value
    let resp = app
...
async fn test_custom_status_board_mapping() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/statuses",
            &tok,
            Some(json!({"name":"qa_review","category":"in_progress"})),
        ))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/statuses",
            &tok,
            Some(json!({"name":"deployed","category":"done"})),
        ))
        .await
        .unwrap();
    ...
    assert!(
        board["in_progress"]
            ...
            .unwrap()
            .iter()
            .any(|t| t["title"] == "BoardTask"),
        "Custom status with category in_progress should map to in_progress column"
    );
    ...
    assert!(
        board["done"]
            ...
            .iter()
            .any(|t| t["title"] == "BoardTask"),
        "Custom status with category done should map to done column"
    );
}
...
async fn test_custom_status_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/statuses",
            &tok,
            Some(json!({"name":"review","color":"#f59e0b","category":"in_progress"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/statuses", &tok, None))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "PUT",
            &format!("/api/statuses/{}", sid),
            &tok,
            Some(json!({"name":"code_review","color":"#10b981","category":"in_progress"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_custom_status_duplicate_rejected() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/statuses",
            &tok,
            Some(json!({"name":"dup_status","category":"todo"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_custom_status_non_root_cannot_create() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/statuses",
            &user_tok,
            Some(json!({"name":"mystat","category":"todo"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
...
async fn test_custom_status_unknown_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", tid),
            &tok,
            Some(json!({"status":"nonexistent_status"})),
        ))
        .await
        .unwrap();
    assert_eq!(
        resp.status(),
        400,
        "Unknown custom status should be rejected"
    );
}
...
async fn test_custom_status_used_in_task() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/statuses",
            &tok,
            Some(json!({"name":"testing","category":"in_progress"})),
        ))
        .await
        .unwrap();
    // Create task
...
async fn test_estimation_accuracy() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"est task","estimated":5})),
        ))
        .await
        .unwrap();
    let task_id = body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", task_id),
            &tok,
            Some(json!({"status":"completed","actual":4})),
        ))
        .await
        .unwrap();
    // Get accuracy
...
async fn test_estimation_accuracy_project_filter() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"proj task","estimated":3,"project":"alpha"})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}", id),
            &tok,
            Some(json!({"status":"completed","actual":3})),
        ))
        .await
        .unwrap();
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/analytics/estimation-accuracy?project=alpha",
            &tok,
            None,
        ))
        .await
...
async fn test_feed_includes_comments() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"feed comment task"})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/comments", tid),
            &tok,
            Some(json!({"content":"test comment for feed"})),
        ))
        .await
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/feed?types=comment", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_focus_score() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/analytics/focus-score", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_leaderboard() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/leaderboard", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_list_notifications() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/notifications", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let j = body_json(resp).await;
...
async fn test_priority_suggestions() {
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"overdue","due_date":"2020-01-01"})),
        ))
        .await
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/suggestions/priorities", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
    ...
    assert!(suggestion["reasons"]
        ...
        .unwrap()
        .iter()
        .any(|r| r.as_str().unwrap().contains("Overdue")));
}
...
async fn test_profile_email_update() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/admin/users", &tok, None))
        .await
        .unwrap();
    let users = body_json(resp).await;
    let root = users
...
async fn test_schedule_suggestions() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/suggestions/schedule", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_shared_session_cannot_join_own() {
    ...
    let resp = app
        ...
        .await
        .unwrap();
    let session_id = body_json(resp).await["current_session_id"]
        .as_i64()
        .unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/timer/join/{}", session_id),
            &tok,
            None,
        ))
        .await
...
async fn test_shared_session_join() {
    ...
    let reg_resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"joiner_f11","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    ...
    let session_id = state["current_session_id"]
        .as_i64()
        .expect("current_session_id should be set after start");
    // Second user joins
    let resp = app
        .clone()
        .oneshot(auth_req(
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/timer/participants/{}", session_id),
            &tok,
            None,
        ))
        .await
...
async fn test_shared_session_not_found() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/timer/join/99999", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 404);
}
...
async fn test_sla_report() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"SLA1","priority":5})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/reports/sla", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let report = body_json(resp).await;
    assert!(!report["resolution_time_by_priority"]
        .as_array()
        .unwrap()
        .is_empty());
...
async fn test_sla_report_non_admin_rejected() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/reports/sla", &user_tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
}
...
async fn test_sprint_retro_report() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"retro task","estimated":3})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/sprints/{}/retro-report", sid),
            &tok,
            None,
        ))
        .await
...
async fn test_sprint_retro_report_not_found() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/sprints/99999/retro-report",
            &tok,
            None,
        ))
        .await
...
async fn test_standup_report() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/reports/standup", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let report = body_json(resp).await;
...
async fn test_unread_count() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/notifications/unread", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let j = body_json(resp).await;
...
async fn test_user_presence() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/users/presence", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
...
async fn test_weekly_digest() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/reports/weekly-digest", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    let data = body_json(resp).await;
