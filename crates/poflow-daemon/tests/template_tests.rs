...
async fn test_templates_crud() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/templates", &tok, None))
        .await
        .unwrap();
...
async fn test_template_create_and_delete() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/templates",
            &tok,
            Some(json!({"name":"Bug Report","data":"{\"fields\":[\"title\",\"steps\"]}"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_template_limits() {
    ...
    // Data too large
    let big_data: serde_json::Value =
        serde_json::from_str(&format!("{{\"x\":\"{}\"}}", "y".repeat(70000))).unwrap();
    let resp = app
        .clone()
        .oneshot(auth_req(
            "POST",
            "/api/templates",
            &tok,
            Some(json!({"name": "big", "data": big_data})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/templates",
            &tok,
            Some(json!({"name": "ok", "data": {"title":"T"}})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_save_task_as_template_captures_labels() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/labels",
            &tok,
            Some(json!({"name":"tmpl_save_label","color":"#22c55e"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/save-as-template", tid),
            &tok,
            None,
        ))
        .await
...
async fn test_template_instantiate_with_custom_fields() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/fields",
            &tok,
            Some(json!({"name":"tmpl_env","field_type":"text"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/templates/{}/instantiate", tmpl_id),
            &tok,
            None,
        ))
        .await
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/tasks/{}/fields", new_tid),
            &tok,
            None,
        ))
        .await
    ...
    assert!(fields
        ...
        .unwrap()
        .iter()
        .any(|f| f["field_name"] == "tmpl_env" && f["value"] == "production"));
}
...
async fn test_template_instantiate_with_labels() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/labels",
            &tok,
            Some(json!({"name":"tmpl_bug","color":"#ef4444"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    assert!(new_task["title"]
        .as_str()
        .unwrap()
        .starts_with("Bug: 2026-"));
    // Verify label was applied
    let resp = app
        .clone()
        .oneshot(auth_req(
            "GET",
            &format!("/api/tasks/{}/labels", new_tid),
            &tok,
            None,
        ))
        .await
    ...
    assert!(labels
        ...
        .unwrap()
        .iter()
        .any(|l| l["name"] == "tmpl_bug"));
    // Verify checklist
    let resp = app
        .clone()
        .oneshot(auth_req(
            "GET",
            &format!("/api/tasks/{}/checklist", new_tid),
            &tok,
            None,
        ))
        .await
...
async fn test_template_save_and_instantiate_with_checklist() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Sprint Review","project":"Team","priority":4})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    app.clone()
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"title":"Prepare slides"})),
        ))
        .await
        .unwrap();
    app.clone()
    ...
    assert_eq!(
        ...
        2,
        "Checklist should be copied from template"
    );
