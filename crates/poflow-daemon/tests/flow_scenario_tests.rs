...
mod common;
use common::{app, auth_req, body_json, json_req, login_root, register_user_full};
...
async fn flow_register_creates_user_role() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/timer", &token, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
}
...
async fn flow_register_weak_password_rejected() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"u1","password":"Ab1"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"u2","password":"abcdefg1"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"u3","password":"Abcdefgh"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn flow_register_duplicate_rejected() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"dup","password":"TestPass1"})),
        ))
        .await
        .unwrap();
...
async fn flow_register_disabled_via_env() {
    ...
    let config = poflow_daemon::config::Config {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"blocked","password":"Block1234"})),
        ))
        .await
        .unwrap();
...
async fn flow_refresh_token_rejected_as_access() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/timer", refresh, None))
        .await
        .unwrap();
...
async fn flow_logout_revokes_access_token() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/auth/logout", &token, None))
        .await
        .unwrap();
...
async fn flow_assignee_cannot_unassign_self() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/assignees/dev2", tid),
            &_user_token,
            None,
        ))
        .await
...
async fn flow_normal_user_cannot_update_others_task() {
    ...
    let tid = body_json(resp).await["id"]
        .as_i64()
        .expect("task should have id");
...
async fn flow_elevate_user_to_root() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "PUT",
            &format!("/api/admin/users/{}/role", uid),
            &root_token,
            Some(json!({"role":"root"})),
        ))
        .await
...
async fn flow_cannot_demote_last_root() {
    ...
    assert_eq!(
        resp.status(),
        400,
        "Should not be able to demote the last root user"
    );
}
...
async fn flow_role_change_creates_audit_log() {
    ...
    assert!(
        arr.iter().any(|e| e["action"] == "update_role"),
        "Audit log should contain role change entry"
    );
}
...
async fn flow_cannot_delete_last_root() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("DELETE", "/api/admin/users/1", &root_token, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
}
...
async fn flow_deleted_user_cannot_login() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"gone","password":"GonePass1"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 401);
...
async fn flow_admin_reset_password() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "PUT",
            &format!("/api/admin/users/{}/password", uid),
            &root_token,
            Some(json!({"password":"NewPass11"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 204);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"resetme","password":"NewPass11"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"resetme","password":"OldPass11"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 401);
...
async fn flow_sprint_full_lifecycle() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &root_token,
            Some(json!({"title":"Sprint task","estimated":3})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &root_token,
            Some(json!({"name":"S1","start_date":"2026-04-14","end_date":"2026-04-28"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/sprints/{}/complete", sid),
            &root_token,
            None,
        ))
        .await
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/sprints/{}/burn", sid),
            &root_token,
            Some(json!({"task_id":tid,"hours":1.0})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn flow_room_full_estimation_session() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/rooms/{}/start-voting", rid),
            &root_token,
...
async fn flow_timer_multi_user_isolation() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/timer", &u_token, None))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/timer", &root_token, None))
        .await
        .unwrap();
    assert_eq!(body_json(resp).await["status"], "Running");
}
...
async fn flow_timer_pause_resume_stop() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/timer/pause", &token, None))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/timer/resume", &token, None))
        .await
        .unwrap();
    assert_eq!(body_json(resp).await["status"], "Running");
    // Stop
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/timer/stop", &token, None))
        .await
        .unwrap();
...
async fn flow_change_password_via_profile() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &token,
            Some(json!({"password":"NewPass11","current_password":"OldPass11"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"pwuser","password":"NewPass11"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn flow_change_password_wrong_current_rejected() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/profile",
            &token,
            Some(json!({"password":"NewPass22","current_password":"WrongOld1"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
...
async fn flow_labels_require_task_ownership() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/labels",
            &root_token,
            Some(json!({"name":"flowbug","color":"#ff0000"})),
        ))
        .await
        .unwrap();
...
async fn flow_soft_delete_restore_cycle() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/tasks/trash", &token, None))
        .await
        .unwrap();
...
async fn flow_backup_non_root_rejected() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/admin/backup", &user_token, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
}
...
async fn flow_history_user_scoped() {
    ...
    app.clone()
        ...
        .unwrap();
    app.clone()
        .oneshot(auth_req("POST", "/api/timer/stop", &root_token, None))
        .await
        .unwrap();
    app.clone()
        .oneshot(auth_req(
        ...
        .unwrap();
    app.clone()
        .oneshot(auth_req("POST", "/api/timer/stop", &user_token, None))
        .await
        .unwrap();
    // User should only see own history
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/history", &user_token, None))
        .await
        .unwrap();
    ...
    for s in arr {
        let user = s["session"]["user"]
            .as_str()
            .unwrap_or(s["user"].as_str().unwrap_or(""));
...
async fn test_full_project_workflow() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Backend","project":"MyApp","priority":4})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Frontend","project":"MyApp","priority":3})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Deploy","project":"MyApp","priority":5,"parent_id":t1})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/labels",
            &tok,
            Some(json!({"name":"wf_critical","color":"#ef4444"})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/dependencies", t3),
            &tok,
            Some(json!({"depends_on":t1})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/checklist", t1),
            &tok,
            Some(json!({"title":"Write API"})),
        ))
        .await
        .unwrap();
    app.clone()
    ...
    assert_eq!(
        board["done"].as_array().unwrap().len(),
        2,
    ...
    app.clone()
        .oneshot(auth_req(
            ...
            &format!("/api/sprints/{}/burn", sid),
            &tok,
            Some(json!({"task_id":t1,"hours":4.0})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            "/api/export/project?project=MyApp",
            &tok,
            None,
        ))
        .await
...
async fn test_multi_user_collaboration() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"collab_admin","password":"CollAdm11"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/tasks/{}/comments", tid),
            &user_tok,
            Some(json!({"content":"Working on it"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/assignees/collab_dev", tid),
            &user_tok,
            None,
        ))
        .await
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/admin/users", &admin_tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
}
