...
mod common;
use common::{app, auth_req, body_json, json_req, login_root, reg};
...
async fn test_room_create_and_list() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/rooms",
            &tok,
            Some(json!({"name":"Sprint 1","estimation_unit":"points"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .oneshot(auth_req("GET", "/api/rooms", &tok, None))
        .await
        .unwrap();
...
async fn test_room_full_voting_flow() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/rooms",
            &tok,
            Some(json!({"name":"R","estimation_unit":"points"})),
        ))
        .await
        .unwrap();
...
async fn test_room_join_leave_kick() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"eve","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/rooms",
            &tok,
            Some(json!({"name":"R","estimation_unit":"hours"})),
        ))
        .await
        .unwrap();
    let rid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", &format!("/api/rooms/{}", rid), &tok, None))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/rooms/{}/members/eve", rid),
            &tok,
            None,
        ))
        .await
    ...
    let resp = app
        ...
        .unwrap();
    assert_eq!(
        body_json(resp).await["members"].as_array().unwrap().len(),
        1
    );
}
...
async fn test_room_role_promotion() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"dan","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "PUT",
            &format!("/api/rooms/{}/role", rid),
            &tok,
            Some(json!({"username":"dan","role":"admin"})),
        ))
        .await
        .unwrap();
    assert!(resp.status().is_success());
...
async fn test_room_non_admin_cannot_start_voting() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"noob","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_get_room_no_auto_join() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"viewer","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", &format!("/api/rooms/{}", rid), &tok2, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 403);
...
async fn test_delete_room_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"roomuser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_room_ws_auth() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/timer/ticket", &tok, None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_room_invalid_estimation_unit() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/rooms",
            &tok,
            Some(json!({"name":"R","estimation_unit":"bananas"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_room_mandays_estimation() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/rooms",
            &tok,
            Some(json!({"name":"MandayRoom","estimation_unit":"mandays"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_concurrent_room_voting() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"voter2","password":"Pass1234"})),
        ))
        .await
        .unwrap();
...
async fn test_room_membership_filter() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"roomuser","password":"Password1!"})),
        ))
        .await
        .unwrap();
    let resp = app
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/rooms", &user_tok, None))
        .await
        .unwrap();
...
async fn test_kick_member() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/rooms/{}/members/kickee", rid),
            &tok,
            None,
        ))
        .await
