...
async fn test_register_and_login() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"alice","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_unauthenticated_rejected() {
    let app = app().await;
    let resp = app
        .oneshot(json_req("GET", "/api/tasks", None))
        .await
        .unwrap();
    assert_eq!(resp.status(), 401);
}
...
async fn test_password_min_length() {
    ...
    let resp = app
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"short","password":"abc"})),
        ))
        .await
        .unwrap();
...
async fn test_password_complexity_uppercase() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"nocase","password":"pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"nodigit","password":"Password"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_logout_revokes_token() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"logoutuser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("POST", "/api/auth/logout", &tok, None))
        .await
        .unwrap();
...
async fn test_refresh_token() {
    ...
    let refresh = body["refresh_token"]
        .as_str()
        .unwrap_or_else(|| panic!("No refresh_token in: {}", body))
        .to_string();
...
async fn test_refresh_token_rejected_as_access() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"root","password":"root1234"})),
        ))
        .await
        .unwrap();
    let body = body_json(resp).await;
    ...
    if refresh.is_empty() {
        return;
    } // skip if no refresh token support
...
async fn test_csrf_header_required() {
    ...
    let req = Request::builder()
        ...
        .body(Body::from(
            serde_json::to_vec(&json!({"title":"T"})).unwrap(),
        ))
        .unwrap();
...
async fn test_sse_ticket_exchange() {
    ...
    let resp = app
        ...
        .oneshot(
            Request::builder()
                .method("GET")
                .uri(format!("/api/timer/sse?ticket={}", ticket))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn test_auth_rate_limiting() {
    ...
    for i in 0..10 {
        let resp = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&json!({"username":"root","password":"wrong"})).unwrap(),
                    ))
                    .unwrap(),
            )
            .await
        ...
        if i < 10 {
            assert_eq!(resp.status(), 401, "Request {} should be 401", i);
        }
    }
...
async fn test_rate_limiter_no_ip_header() {
    ...
    let req = axum::http::Request::builder()
        ...
        .body(axum::body::Body::from(
            serde_json::to_string(&json!({"username":"root","password":"root1234"})).unwrap(),
        ))
        .unwrap();
    let resp = app.clone().oneshot(req).await.unwrap();
...
async fn test_update_username_uniqueness() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"unique1","password":"Pass1234"})),
        ))
        .await
        .unwrap();
...
async fn test_username_validation() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":long,"password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"bad user!","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"good_user-1","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_login_succeeds_after_register() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"rehashuser","password":"Testpass1"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_register_duplicate_username() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"dupUser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 409);
...
async fn test_auth_full_flow() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"flowuser","password":"Flow1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/login",
            Some(json!({"username":"flowuser","password":"wrong"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 401);
...
async fn test_auth_rate_limit_threshold() {
    ...
    for i in 0..12 {
        ...
        if i < 10 {
            assert_eq!(last_status, 401, "Attempt {} should be 401", i);
        }
    }
    assert_eq!(
        last_status, 429,
...
async fn test_read_rate_limiter_enforced() {
    ...
    assert!(
        ...
        "GET rate limiter should reject after 1000 requests"
    );
}
...
async fn test_refresh_token_rotation() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"refreshUser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let auth = body_json(resp).await;
