...
async fn test_dependencies_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/dependencies", b),
            &tok,
            Some(json!({"depends_on": a})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/dependencies", &tok, None))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/dependencies/{}", b, a),
            &tok,
            None,
        ))
        .await
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/dependencies", a),
            &tok,
            Some(json!({"depends_on": a})),
        ))
        .await
...
async fn test_dependency_crud_and_list() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/dependencies", id1),
            &tok,
            Some(json!({"depends_on":id2})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/tasks/{}/dependencies/{}", id1, id2),
            &tok,
            None,
        ))
        .await
...
async fn test_dependency_self_reference() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/tasks/{}/dependencies", tid),
            &tok,
            Some(json!({"depends_on": tid})),
        ))
        .await
        .unwrap();
...
async fn test_dependency_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"depuser","password":"Password1!"})),
        ))
        .await
        .unwrap();
    let resp = app
