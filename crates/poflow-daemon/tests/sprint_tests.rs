...
mod common;
use common::{app, auth_req, body_json, json_req, login_root};
...
async fn test_sprint_create_and_list() {
    ...
    let resp = app
        .oneshot(auth_req("GET", "/api/sprints", &tok, None))
        .await
        .unwrap();
...
async fn test_sprint_update() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &tok,
            Some(json!({"name":"S2","goal":"New goal"})),
        ))
        .await
        .unwrap();
...
async fn test_sprint_filter_by_status() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/sprints?status=active", &tok, None))
        .await
        .unwrap();
...
async fn test_sprint_add_remove_tasks() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/sprints/{}/tasks/{}", sid, t1),
            &tok,
            None,
        ))
        .await
...
async fn test_sprint_burndown_and_snapshot() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T","estimated_hours":8.0})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/sprints/{}/burndown", sid),
            &tok,
            None,
        ))
        .await
...
async fn test_sprint_root_tasks() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Child","parent_id":parent_id})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            "POST",
            &format!("/api/sprints/{}/roots", sid),
            &tok,
            Some(json!({"task_ids":[parent_id]})),
        ))
        .await
        .unwrap();
    ...
    app.clone()
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/sprints/{}/roots/{}", sid, parent_id),
            &tok,
            None,
        ))
        .await
...
async fn test_update_sprint_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"sprintuser2","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_delete_sprint_ownership() {
    ...
    app.clone()
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"sprintuser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let resp = app
...
async fn test_sprint_task_auth() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"sprinteve","password":"Sprinteve1"})),
        ))
        .await
        .unwrap();
    assert!(
...
async fn test_snapshot_sprint_points_not_double_counted() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T","remaining_points":5.0,"estimated":3})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
...
async fn test_optimistic_locking_sprint() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/sprints/{}", id),
            &tok,
            Some(json!({"name":"S2","expected_updated_at":updated_at})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/sprints/{}", id),
            &tok,
            Some(json!({"name":"S3","expected_updated_at":updated_at})),
        ))
        .await
        .unwrap();
...
async fn test_sprint_date_validation() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"S","start_date":"2026"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"S","start_date":"2026-04-11"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let task = body_json(
        app.clone()
            .oneshot(auth_req(
                ...
                "/api/tasks",
                &tok,
                Some(json!({"title":"T","remaining_points":5.0})),
            ))
            .await
            .unwrap(),
    )
...
async fn test_sprint_retro_notes() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/sprints/{}", sid),
            &tok,
            Some(json!({"retro_notes":"Good sprint!"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_sprint_lifecycle() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"Sprint1","goal":"Ship it"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
    ...
    let board = body_json(resp).await;
    assert!(
        board["todo"].as_array().unwrap().len()
            + board["in_progress"].as_array().unwrap().len()
...
async fn test_sprint_date_ordering() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"BadDates","start_date":"2026-04-20","end_date":"2026-04-10"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"GoodDates","start_date":"2026-04-10","end_date":"2026-04-20"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_sprint_date_ordering_create() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"BadDates","start_date":"2025-03-01","end_date":"2025-02-01"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
...
async fn test_sprint_completion_takes_snapshot() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"SprintTask","remaining_points":5.0})),
        ))
        .await
        .unwrap();
    let tid = body_json(resp).await["id"].as_i64().unwrap();
...
async fn test_sprint_carryover() {
    ...
    let t1 = body_json(
        app.clone()
            .oneshot(auth_req(
                ...
                "/api/tasks",
                &tok,
                Some(json!({"title":"CarryDone","status":"completed"})),
            ))
            .await
            .unwrap(),
    )
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "POST",
            &format!("/api/sprints/{}/carryover", sid),
            &tok,
            None,
        ))
        .await
...
async fn test_sprint_capacity() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"CapSprint","capacity_hours":40.0})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
...
async fn test_sprint_root_task_auth() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"noroot","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    let tok2 = body_json(resp).await["token"].as_str().unwrap().to_string();
    ...
    let sid = body_json(
        app.clone()
            .oneshot(auth_req(
                ...
                "/api/sprints",
                &tok,
                Some(json!({"name":"CapSprint","capacity_hours":42.0})),
            ))
            .await
            .unwrap(),
    )
...
async fn test_burndown_snapshot_uses_remaining_points() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"T1","remaining_points":8.0,"estimated":3,"estimated_hours":4.0})),
        ))
        .await
        .unwrap();
    let t1 = body_json(resp).await["id"].as_i64().unwrap();
...
async fn test_sprint_board_columns_reflect_statuses() {
    ...
    // blocked: blocked
    // done: completed, done
    let todo_ids: Vec<i64> = board["todo"]
        .as_array()
        .unwrap()
        .iter()
        .map(|t| t["id"].as_i64().unwrap())
        .collect();
    let wip_ids: Vec<i64> = board["in_progress"]
        .as_array()
        .unwrap()
        .iter()
        .map(|t| t["id"].as_i64().unwrap())
        .collect();
    let blocked_ids: Vec<i64> = board["blocked"]
        .as_array()
        .unwrap()
        .iter()
        .map(|t| t["id"].as_i64().unwrap())
        .collect();
    let done_ids: Vec<i64> = board["done"]
        .as_array()
        .unwrap()
        .iter()
        .map(|t| t["id"].as_i64().unwrap())
    ...
    assert!(
        todo_ids.contains(&t_backlog),
        "backlog task should be in todo column"
    );
    assert!(
        todo_ids.contains(&t_estimated),
        "estimated task should be in todo column"
    );
    assert!(
        wip_ids.contains(&t_active),
        "active task should be in in_progress column"
    );
    assert!(
        wip_ids.contains(&t_wip),
        "in_progress task should be in in_progress column"
    );
    assert!(
        blocked_ids.contains(&t_blocked),
        "blocked task should be in blocked column"
    );
    assert!(
        done_ids.contains(&t_completed),
        "completed task should be in done column"
    );
    assert!(
        done_ids.contains(&t_done),
    ...
    let todo_ids: Vec<i64> = board["todo"]
        ...
        .collect();
    assert!(
        wip_ids.contains(&t_backlog),
        "moved task should now be in in_progress column"
    );
    assert!(
        !todo_ids.contains(&t_backlog),
        "moved task should no longer be in todo column"
    );
    ...
    let blocked_ids: Vec<i64> = board["blocked"]
        ...
        .collect();
    assert!(
        done_ids.contains(&t_blocked),
        "unblocked task should be in done column"
    );
    assert!(
        !blocked_ids.contains(&t_blocked),
        "unblocked task should not be in blocked column"
    );
}
...
async fn test_compare_sprints() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"SprintA","start_date":"2026-01-01","end_date":"2026-01-14"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/sprints",
            &tok,
            Some(json!({"name":"SprintB","start_date":"2026-01-15","end_date":"2026-01-28"})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "GET",
            &format!("/api/sprints/compare?a={}&b={}", a, b),
            &tok,
            None,
        ))
        .await
