...
async fn test_epic_groups_crud() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/tasks",
            &tok,
            Some(json!({"title":"Epic Root","estimated":5})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/epics", &tok, None))
        .await
        .unwrap();
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", &format!("/api/epics/{}", eid), &tok, None))
        .await
        .unwrap();
...
async fn test_epic_group_task_management() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            &format!("/api/epics/{}/tasks", eid),
            &tok,
            Some(json!({"task_ids":[t1["id"],t2["id"]]})),
        ))
        .await
        .unwrap();
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            "DELETE",
            &format!("/api/epics/{}/tasks/{}", eid, t1["id"]),
            &tok,
            None,
        ))
        .await
...
async fn test_epic_group() {
    ...
    let resp = app
        ...
        .oneshot(auth_req(
            ...
            "/api/epics",
            &tok,
            Some(json!({"name":"Epic1","description":"Test epic"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 201);
