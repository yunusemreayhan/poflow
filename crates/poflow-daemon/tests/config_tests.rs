...
async fn test_config_get() {
    ...
    let resp = app
        .oneshot(auth_req("GET", "/api/config", &tok, None))
        .await
        .unwrap();
...
async fn test_user_config_override() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("PUT", "/api/config", &tok, Some(new_cfg)))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
...
async fn test_config_validation_bounds() {
    ...
    bad["work_duration_min"] = json!(999);
    let resp = app
        .clone()
        .oneshot(auth_req("PUT", "/api/config", &tok, Some(bad)))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    // daily_goal too high
    let mut bad = base.clone();
    bad["daily_goal"] = json!(100);
    let resp = app
        .clone()
        .oneshot(auth_req("PUT", "/api/config", &tok, Some(bad)))
        .await
    ...
    let mut bad = base.clone();
    bad["estimation_mode"] = json!("invalid");
    let resp = app
        .clone()
        .oneshot(auth_req("PUT", "/api/config", &tok, Some(bad)))
        .await
...
async fn test_per_user_config_isolation() {
    ...
    let resp = app
        ...
        .oneshot(json_req(
            "POST",
            "/api/auth/register",
            Some(json!({"username":"configUser","password":"Pass1234"})),
        ))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/config", &root_tok, None))
        .await
        .unwrap();
    let root_cfg = body_json(resp).await;
    let resp = app
        .clone()
        .oneshot(auth_req("GET", "/api/config", &user_tok, None))
        .await
        .unwrap();
    ...
    let user_cfg = body_json(resp).await;
    assert_eq!(
        user_cfg["work_duration_min"], 15,
        "user should see their override"
    );
    ...
    let root_cfg = body_json(resp).await;
    assert_eq!(
        root_cfg["work_duration_min"], 25,
        "root should still see global default"
    );
...
async fn test_config_all_bounds() {
    ...
    let mut bad = cfg.clone();
    bad["work_duration_min"] = json!(241);
    let resp = app
        .clone()
        .oneshot(auth_req("PUT", "/api/config", &tok, Some(bad)))
        .await
...
async fn test_config_theme_validation() {
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("PUT", "/api/config", &tok, Some(cfg.clone())))
        .await
        .unwrap();
    assert_eq!(resp.status(), 400);
    ...
    let resp = app
        .clone()
        .oneshot(auth_req("PUT", "/api/config", &tok, Some(cfg)))
        .await
        .unwrap();
    assert_eq!(resp.status(), 200);
}
