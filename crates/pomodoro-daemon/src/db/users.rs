...
pub async fn delete_user(pool: &Pool, id: i64) -> Result<()> {
    ...
    sqlx::query("UPDATE sessions SET user_id = (SELECT id FROM users WHERE role = 'root' AND id != ? LIMIT 1) WHERE user_id = ?").bind(id).bind(id).execute(&mut *tx).await?;
    sqlx::query("UPDATE tasks SET user_id = (SELECT id FROM users WHERE role = 'root' AND id != ? LIMIT 1) WHERE user_id = ?").bind(id).bind(id).execute(&mut *tx).await?;
