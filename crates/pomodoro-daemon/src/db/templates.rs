...
pub async fn list_templates(pool: &Pool, user_id: i64) -> Result<Vec<TaskTemplate>> {
    Ok(sqlx::query_as("SELECT * FROM task_templates WHERE user_id = ? ORDER BY name")
        .bind(user_id).fetch_all(pool).await?)
}

pub async fn create_template(pool: &Pool, user_id: i64, name: &str, data: &str) -> Result<TaskTemplate> {
    let now = now_str();
    let id = sqlx::query("INSERT INTO task_templates (user_id, name, data, created_at) VALUES (?, ?, ?, ?)")
        .bind(user_id).bind(name).bind(data).bind(&now).execute(pool).await?.last_insert_rowid();
