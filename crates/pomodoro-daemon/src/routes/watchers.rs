...
pub async fn add_task_dependency(State(engine): State<AppState>, claims: Claims, Path(id): Path<i64>, Json(req): Json<serde_json::Value>) -> Result<StatusCode, ApiError> {
...
pub async fn remove_task_dependency(State(engine): State<AppState>, claims: Claims, Path((id, dep_id)): Path<(i64, i64)>) -> Result<StatusCode, ApiError> {
