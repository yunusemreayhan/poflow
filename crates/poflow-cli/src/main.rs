...
async fn api(
    ...
    if method != "GET" {
        req = req.header("x-requested-with", "pomo-cli");
    }
    if let Some(b) = body {
        req = req.json(&b);
    }
...
async fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Pause => {
            api(&client, base, token, "POST", "/api/timer/pause", None).await?;
            println!("Paused");
        }
        Cmd::Resume => {
            api(&client, base, token, "POST", "/api/timer/resume", None).await?;
            println!("Resumed");
        }
        Cmd::Stop => {
            api(&client, base, token, "POST", "/api/timer/stop", None).await?;
            println!("Stopped");
        }
        Cmd::Skip => {
            api(&client, base, token, "POST", "/api/timer/skip", None).await?;
            println!("Skipped");
        }
        Cmd::Tasks { status } => {
            let path = match status {
                Some(ref s) => format!("/api/tasks?status={}", s),
        ...
        Cmd::Stats => {
            ...
            if let Some(arr) = stats.as_array() {
                for s in arr {
                    println!(
                        "{}: {} completed, {} interrupted, {}m focus",
                        s["date"].as_str().unwrap_or("?"),
                        s["completed"],
                        s["interrupted"],
                        s["total_focus_s"].as_i64().unwrap_or(0) / 60
                    );
                }
            }
        }
        Cmd::Sprints { status } => {
            let path = match status {
                Some(ref s) => format!("/api/sprints?status={}", s),
                None => "/api/sprints".to_string(),
            };
        ...
        Cmd::Deps { task_id } => {
            let deps = api(
                ...
                token,
                "GET",
                &format!("/api/tasks/{}/dependencies", task_id),
                None,
            )
            .await?;
        ...
        Cmd::Log {
            ...
            api(
                ...
                token,
                "POST",
                &format!("/api/tasks/{}/time", task_id),
                Some(json!({"hours": hours, "description": note})),
            )
            .await?;
        ...
        Cmd::Score => {
            let score = api(
                ...
                token,
                "GET",
                "/api/analytics/focus-score",
                None,
            )
            .await?;
            println!(
                "Focus Score: {}/100 (streak: {}d)",
                score["score"], score["streak_days"]
            );
            ...
            if let Some(arr) = rooms.as_array() {
                for r in arr {
                    println!(
                        ...
                        r["estimation_unit"].as_str().unwrap_or("?")
                    );
                }
            }
        }
        ...
        Cmd::Search { query } => {
            ...
            if let Some(tasks) = results["tasks"].as_array() {
                for t in tasks {
                    println!("[task] #{} {}", t["id"], t["title"].as_str().unwrap_or("?"));
                }
            }
        ...
        Cmd::Show { task_id } => {
            ...
            if let Some(p) = t["project"].as_str() {
                println!("  Project: {}", p);
            }
            if let Some(d) = t["due_date"].as_str() {
                println!("  Due: {}", d);
            }
            if let Some(comments) = detail["comments"].as_array() {
                for c in comments {
                    println!(
                        ...
                        &c["created_at"].as_str().unwrap_or("?")[..10],
        ...
        Cmd::Checklist { task_id } => {
            let items = api(
                ...
                token,
                "GET",
                &format!("/api/tasks/{}/checklist", task_id),
                None,
            )
            .await?;
