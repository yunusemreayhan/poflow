...
sqlite3 poflow.db ".backup backup_$(date +%Y%m%d).db"

# Or copy the file (stop server first for consistency)
cp poflow.db poflow_backup_$(date +%Y%m%d).db
```
...
cp backup_20260411.db poflow.db
# Restart the server
```
...
sqlite3 poflow.db "SELECT id, username, role, created_at FROM users;"

# Change user role
sqlite3 poflow.db "UPDATE users SET role = 'root' WHERE username = 'admin';"
...
RUST_LOG=debug cargo run -p poflow-daemon

# Filter for errors only
RUST_LOG=error cargo run -p poflow-daemon

# Check for rate limiting hits
RUST_LOG=warn cargo run -p poflow-daemon 2>&1 | grep "Too many"
```
...
sqlite3 poflow.db "PRAGMA integrity_check;"

# Vacuum (reclaim space after many deletes)
sqlite3 poflow.db "VACUUM;"

# Check database size
ls -lh poflow.db

# Prune expired token blocklist entries
sqlite3 poflow.db "DELETE FROM token_blocklist WHERE expires_at < datetime('now');"
```

## Configuration

Config file: `~/.config/poflow/config.toml`

```bash
# View current config
cat ~/.config/poflow/config.toml

# Reset to defaults (delete config, server recreates on start)
rm ~/.config/poflow/config.toml
```
